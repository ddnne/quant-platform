#!/usr/bin/env python3
"""W107 / w0820d Track E — research-family APPEND (NOT promotion).

Append **this-wave newly min-implemented logics only**.
generation_enabled=false. No auto research_candidate.
...
Examples
--------
    uv run python scripts/run_w107_research_family_append.py \\
...
import run_w103_dispersion_deepen as w103d  # noqa: E402
import run_w107_new_hyps_daily_dd as w107b  # noqa: E402
import run_w107_funding_surprise_adaptive as w107c  # noqa: E402
...
def run_append(*, out_dir: Path, seed: int, log) -> dict[str, Any]:
    ...
    this_wave = list(w107b.NEW_UNIQUE_LOGIC) + list(w107c.ADAPTIVE_VARIANTS)
    _dump(
    ...
    proposals = w107b.proposals_for_factory() + w107c.proposals_for_factory()
    cfg = MassFactoryConfig(seed=int(seed), n=max(20, len(proposals) + 5))
    log(
        f"[w107/E] append_id={RESEARCH_FAMILY_APPEND_ID} this_wave_only "
    ...
    summary = {
        "wave": WAVE,
        "track": "E_research_family_append",
        ...
        "this_wave_new": [s.get("logic_id") for s in this_wave],
    ...
    log(
        f"[w107/E] accepted={summary['n_accepted']} "
...
def confirm_e_no_extra_grids(log) -> dict[str, Any]:
    pack = {
        ...
        "note": (
            "W107 E: no extra dispersion_gate grid. sticky stays "
    ...
    log(
        f"[w107/E] gate={GATE_STANCE} sticky={STICKY_STANCE} "
...
def confirm_f_pins_misdate_projection(log) -> dict[str, Any]:
    ...
    pins["note"] = "W107 E: 3-default pins must remain untouched"
    ...
    log(
        f"[w107/E] pins_untouched={pins.get('pins_untouched')} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    log_path = out_dir / "w107_research_family_append.log"
    ...
    t0 = time.time()
    log(
        "[w107/E] research-family append = recognition, not pass / "
    ...
    _dump(out_dir / "w107_e_summary.json", wrap)
    log(f"[w107/E] done wall={wrap['wall_sec']}s")
