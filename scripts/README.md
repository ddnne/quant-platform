...
is the staging-only, audit-only Receipt ACTIVE collector. It accepts only the
reviewed source SHA, account ID, and Cloudflare API token; the staging key
registry and output directory are fixed. The gate independently brackets the
three-Worker Receipt chain (Secrets, Receipt authority, Premium), remeasures
module bytes and public surfaces, and reads the official management D1
attestation row. It derives `reservation_id` from the current SHA and caller
version, requires exactly one `ATTESTED` row, keeps the exact canonical UTF-8
bytes/digest, and compares D1 `operation_id` and nonce to the verified signed
claims. Evidence format v5 is management-collected signed runtime recovery
evidence, not observer RPC/Access-observed evidence. The public API cannot
accept caller-supplied evidence JSON, a skip flag, or an alternate trust root.
Observer Access helpers in the same module remain for the separate JSDA
release-observation HOLD (`RECEIPT_OBSERVER_ACCESS_CLIENT_ID` /
`RECEIPT_OBSERVER_ACCESS_CLIENT_SECRET`); that path is not this Receipt ACTIVE
collector. A success writes only canonical, content-addressed, create-only
local `AUDIT_ONLY` evidence and cannot authorize Receipt issuance, release,
READY, or Pilot.
