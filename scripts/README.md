...
**CI vs pre-push:** [`verify_all.sh`](verify_all.sh) is the fast local helper (optional `VERIFY_*` worker steps; may skip missing `node_modules`). [`verify_ci.sh`](verify_ci.sh) is the mandatory authoritative CI: it fails if anything is missing — Python 3.11+ `.venv` (no system-python fallback; prints create instructions), `pip install -e ".[dev]"`, `pytest tests/`, catalog compile + `catalog_ids` freeze, Evaluation IR golden/schema presence, all seven workers including `ci-aggregate` (`package-lock.json`, `npm ci`, `npm test`, `npm run typecheck`, `wrangler deploy --dry-run`, `wrangler types --check` if the types script exists else `wrangler types`), tracked `.env`/`*.pem` scan, and a clean git tree after generated types. Never `--legacy-peer-deps`; never skip missing `node_modules`.
