...
def test_empty_raw_and_empty_envelope_are_not_observed_complete() -> None:
    assert count_raw_items(b"") == 0
    assert count_raw_items(b"[]") == 0
    assert count_raw_items(b"{}") == 0
    assert count_raw_items(b'{"data":[]}') == 0
    assert count_raw_items(b'{"rows":[],"results":[]}') == 0
    assert observed_items_from_actual(unit="source_query", raw_item_count=0) == 0
    assert observed_items_from_actual(unit="source_query", raw_item_count=1) == 1
    src = RECEIPTS.read_text(encoding="utf-8")
    assert "empty-raw SUCCESS is forbidden" in src
    assert "Never uses expected" in src
    assert "count_raw_items(raw_bytes)" in src
    assert "observed_items_from_actual" in src
    assert "expected_items" not in src.split("obs = observed_items_from_actual", 1)[1].split("\n", 1)[0]
