...
def test_feature_context_equity_master_post_start_calls_pit():
    """Official island is PIT-readable; PD-D2-MASTER is not Dataset COMPLETE."""
    ...
    ctx.get_equity_master(code="8697")
    assert calls == [("equity_master", {"code": "8697"})]


def test_feature_context_equity_master_pre_official_empty_no_pit():
    def _never_read(resource, kwargs):
        raise AssertionError(f"pre-official master must not load PIT: {resource} {kwargs}")
