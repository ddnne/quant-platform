...
try:
    from storage.coverage_ledger import official_index_days
except ImportError:
    official_index_days = sot_official_index_days
...
_HAS_V3_SPECS = _CAPABILITY.is_file() and _MIGRATION.is_file()


def _planner_accepts_index_text() -> bool:
    import inspect

    return "index_text" in inspect.signature(plan_required_segments).parameters


def _refresh_accepts_index_text() -> bool:
    import inspect

    return "index_text" in inspect.signature(refresh_coverage_ledger).parameters
...
@pytest.mark.skipif(not _HAS_V3_SPECS, reason="source-capability v3 specs not on this slice")
...
@pytest.mark.skipif(not _HAS_V3_SPECS, reason="coverage v3 migration not on this slice")
...
def test_official_index_html_parser_is_one_sot() -> None:
    if official_index_days is not sot_official_index_days:
        pytest.skip("coverage_ledger does not re-export official_index_days on this slice")
...
@pytest.mark.skipif(not _planner_accepts_index_text(), reason="ledger planner has no index_text on this slice")
...
@pytest.mark.skipif(not _refresh_accepts_index_text(), reason="ledger refresh has no index_text on this slice")
...
@pytest.mark.skipif(not _HAS_V3_SPECS, reason="source-capability v3 loader not on this slice")
