...
| Profile/closure-bound READY | The dedicated publisher verifies signed Ops evidence, exact plan closure, PIT availability, immutable DB digest, and the dedicated READY key. Receipt-native pointer publication additionally binds admitted source/physical identity, producer snapshot-quality inventory, unsigned dependency-scope, exact-four plan period, and the committed public READY registry; native execution of that envelope remains rejected | `tests/test_ready_policy_fail_closed.py`, `tests/test_ready_manifest.py`, and `platform/workers/ingestion-premium/src/ready_publication.test.ts` |
