...
**Live strategy at `67fcbd7c`:** §10 — remaining extracts vs HOLD. YAML
...
## 10. Current remaining extracts vs HOLD (HEAD 67fcbd7c)
...
Measured at `67fcbd7c`: tracked paths **756**; catalog YAML
...
`collection_receipts.ts`. `coverage_ledger.py` is **1449**.
`snapshot.py` is **912**.
...
index_text CLIs, OTC grain, Worker R2 stub, and DO `budget_id` pin
are **DONE**. MCP presentation echo and JSDA refresh inventory replay
stay **DONE**. Remaining: IR encode/decode body, opt-in Python TOCTOU,
bounded-history month chunks. Leftover occupancy **HOLD** — do not
...
|---------|--------|--------|
| leftover git worktrees | **Environment waste, not code waste.** Measured **181** `git worktree list` rows at this write (this isolation included); `/private/tmp/qp-*` dirs were **243**. | Prune the environment. Not a module extract. |
