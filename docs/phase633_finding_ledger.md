...
| R5 | Generic READY publication and a same-UID arbitrary READY signer remained reachable | OPEN | **SOURCE-REACHABLE (inactive):** `ceaf9d21`, `e0fa86c4`; the product publisher has one client-only path to the dedicated READY socket, accepts no signer/key fallback, and rebinds the exact profile/closure/snapshot/signed projection before retaining the verified immutable sidecar. Passive registry/socket metadata remains truthfully `PENDING/UNKNOWN`; active keys=0. **OPERATIONAL-OPEN:** provision the READY UID/socket/key/registry/protected store, independently recheck the authenticated current mirror/closure/copy, and publish and verify one immutable exact-four READY; Mass stays disabled. |
