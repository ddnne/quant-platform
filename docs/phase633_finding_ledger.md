...
| D3 | A same-UID importable signing oracle could mint signed SUCCESS outside governed ingestion | OPEN | **SOURCE-PARTIAL (inactive):** `1cdb66c8`, `0ef0035b`, `8bcdc38e`, `CURRENT_CANDIDATE_SHA_PENDING_INTEGRATION`. Public Premium Receipt issue/recover/registration routes are removed. The binding manifest freezes all four named WorkerEntrypoint RPC inventories with reserved `fetch` modeled separately. The Premium operator has no `fetch()` and exposes only argument-free PENDING registration plus a read-only signed `AUDIT_ONLY` staging attestation. The Receipt Durable Object exposes exactly five governed/audit RPCs; all key, state, lookup and event helpers are JavaScript `#private`, and workerd rejects the former helper names with no key/DO/D1/R2 mutation. Registration revalidates authority/action/environment/full source SHA/Worker versions/resource/key identity and canonical operation/registration digests. Normal Receipt issue/recovery and the staging audit canary now use separate modules and narrowed RPC capability types. The canary uses dedicated Durable Object audit state and a three-event initial/first-recovery/replay-confirmation chain: the first recover is unsigned and pending, and only the second identical recover signs the final audit attestation. It cannot call ordinary issue/recover or write collection receipts, Coverage, product data or authority R2. The narrow staging ACTIVE gate owns fixed registry/attestation paths, collects GET-only live documents itself, and uses real Ed25519 verification to remeasure exact deployment brackets, module bytes, bindings, DO migration, secret-name/public surfaces and the signed operation/nonce/initial-state/result/recovery-event/tail/replay-event/tail/final-state chain. It binds an immutable authority-version/caller-version/key/registry pair; any authority, key or registry change requires a later coordinated Premium redeploy and a new version-scoped row, while old attestations are rejected. It cannot grant release, research or `TRUSTED_COLLECTION` eligibility. **SOURCE-PARTIAL/OPERATIONAL-OPEN:** the typed Premium capability exists but no operator principal/caller Worker Service Binding exists. Deploy/accept PENDING, provision the wrap secret/key, capture/review registration through a closed caller, activate the exact scoped registry/staging vars, apply canonical migration `0019`, deploy/run the `AUDIT_ONLY` recovery canary, retire legacy signers and independently review live evidence. The audit canary does not close D2 product reproof. Production also remains C7 HOLD. D2/D3/A2 and the strict release gate remain OPEN. |
