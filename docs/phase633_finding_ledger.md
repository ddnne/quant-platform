...
| R5 | Generic READY publication and a same-UID arbitrary READY signer remained reachable | OPEN | **SOURCE-CONTAINED (inactive):** `ceaf9d21`, `e0fa86c4`, `52a8499d`; the generic candidate engine remains introspectable and is not claimed unreachable. It is non-authoritative, has no production signer/key fallback, and its output cannot pass the isolated signature verifier. Before signing, the dedicated authority descriptor-pins and bounded-copies the artifact, independently recompiles the exact plan/profile/closure, re-verifies signed projection/PIT scope, replays policy, and supplies the measured digest to `VerifiedPilotReadyPublication`. The public production READY SQLite opener is removed from package/snapshot exports; its direct-module compatibility trap rejects before resolver or descriptor-open hooks. A retained `O_RDWR`-before-chmod regression proves `0444` cannot revoke a same-UID writer and patched hooks still cannot return a production connection. The exclusive accepted Controlled path is the activation-owned, root-owned `PinnedControlledSnapshotV2` capability. No general production-read integrity is claimed: QuantData still uses describe-to-path reads, `VerifiedPilotReadyPublication` remains a shallow path-exposing object, and the full READY owner chain plus atomic root-owned READY-to-Controlled install are absent. Hash/copy paths retain 64 GiB and cooperative deadline/free-space bounds; passive registry/socket metadata remains `PENDING/UNKNOWN`, active keys=0. **SOURCE/OPERATIONAL-OPEN with A2/R11:** provision READY UID/socket/key/registry/protected store, implement and accept the root-owned atomic install/custody transition, independently recheck the current mirror/closure/copy, and publish/verify one exact-four READY; Mass stays disabled. |
...
| R11 | Controlled execution duplicated authority lineage into a caller-writable HOME store | OPEN | **SOURCE-REACHABLE (inactive):** `7871bd2c`; six-principal bootstrap/runner binds Controlled to its distinct UID/socket/raw Ed25519 seed/active registry/Trader UID/protected canonical store/immutable snapshot/signed projection activation. The accepted Controlled read type is `PinnedControlledSnapshotV2` and live activation requires root ownership; the general READY SQLite opener is absent from public exports. This does not establish the missing READY owner/custody transition: `VerifiedPilotReadyPublication` exposes paths, QuantData remains a describe-to-path plane, runtime config has no mutable current DB capability, execution/promotion remain false, and active keys=0. **SOURCE/OPERATIONAL-OPEN with A2/R5:** provision the external writer UID/socket/fresh key/active registry/root-owned activation/protected canonical store, implement and accept an atomic root-owned READY-to-Controlled install, then accept one complete authorized Pilot artifact chain. |
...
| A2 | Receipt, D1, Ops, READY, Trader, transition and execution keys had filenames but no complete principal/evidence-authority isolation | OPEN | **SOURCE-PARTIAL (inactive; all six local principals source-runnable; D1 handoff crash-atomic):** `ceaf9d21`, `7871bd2c`; the manifest declares one Cloudflare Receipt authority and six distinct local UIDs/sockets/stores/key backends, and bootstrap/runner/runtime-config/launchd coverage includes Trader and Controlled. The retained `O_RDWR`-before-chmod regression makes the boundary explicit: mode bits, descriptor pinning and rehashing do not isolate a same-UID writer. Public READY SQLite opening is fail-closed, but QuantData describe-to-path reads, `VerifiedPilotReadyPublication` path exposure, and the future root-owned READY-to-Controlled install still require real principal/owner separation. Receipt remains PENDING-only and 0/6 local principals are provisioned. **SOURCE/OPERATIONAL-OPEN with R5/R11:** 0/1 Cloudflare Receipt authority and 0/6 local OS authorities are provisioned; all registries have active keys=0. Add the authority/action/environment/SHA/resource-bound staged gate with research-ineligible canaries, then bootstrap six UIDs/sockets/root-owned stores/fresh in-authority keys, implement the atomic root-owned READY-to-Controlled custody transition, and independently accept all seven protocols plus the full Pilot chain. |
