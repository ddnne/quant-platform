...
| C7 | `ingestion-secrets` workers.dev endpoint is not protected by Access | HOLD | Access API GET organizations returned 9999 `not_enabled`; that does not prove a UI ToS/team-domain prompt. Account enablement needs a user decision only if observed. HOLD unchanged for Worker Access JWT/hostname; header token remains enabled |
...
| A6 | Release evidence existed only at local absolute paths | OPEN | **SOURCE-PARTIAL:** authenticated STAGING JSDA AUDIT_ONLY intake exists in `scripts/build_release_evidence.py`; local digest-named output is `STAGED_LOCAL_NOT_PUBLISHED` / `release_allowed=false` and is not published. Caller JSON remains untrusted. Staging JSDA slice cannot close global A6. Independently accept staging AND production authenticated collection of actual endpoint bytes, then publish a content-addressed non-secret manifest; backup body remains private/encrypted. Local staging is not A6 closure. |
