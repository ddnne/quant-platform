...
| D3 | A same-UID importable signing oracle could mint signed SUCCESS outside governed ingestion | OPEN | **SOURCE-CLOSED (inactive):** `1cdb66c8`, `0ef0035b`, `8bcdc38e`; the closed Service Binding accepts only environment/dataset/segment/nonce; private Ed25519 minting/finalization remains inside the DO and no production importable signer exists. The checked-in Cloudflare PENDING acceptance path pins exact source/environment/authority/config/resources, requires zero active keys, forbids public routes/previews, derives exact named handlers and the Durable Object migration tag from the frozen manifest, and rejects positive RPCs. **OPERATIONAL-OPEN:** deploy and accept the isolated no-public-route Worker/DO/binding in PENDING, provision a fresh secret/key, activate a reviewed registry entry, retire legacy signers, and pass recovery smoke. A local PENDING daemon is deferred because its socket could be mistaken for ACTIVE service evidence during transition. |
