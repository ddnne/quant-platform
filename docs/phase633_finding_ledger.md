...
| A6 | Release evidence existed only at local absolute paths | OPEN | **SOURCE-HOLD:** the production builder rejects every caller-supplied observation document. Authenticated collection/publication implementation is missing (not merely unprovisioned observation keys). `receipt-activation-observer` has a private JSDA `/health/ready` Service Binding collector in source; there is no authenticated client/writer on this publication path. Implement authenticated collection and publication that receive actual endpoint bytes, independently accept staging and production, then publish a content-addressed non-secret manifest; backup body remains private/encrypted. Caller JSON is not remote observation. Collector presence does not close A6. Missing source is distinct from later Access/route activation. |
