...
| R5 | Generic READY publication and a same-UID arbitrary READY signer remained reachable | OPEN | **SOURCE-CONTAINED (inactive):** `ceaf9d21`, `e0fa86c4`, `52a8499d`; the closure-held generic candidate engine remains recoverable through Python introspection and is explicitly non-authoritative rather than a secrecy boundary: it has no production signer/key fallback, and its output cannot pass the isolated signature verifier. Before signing, the dedicated authority descriptor-pins and bounded-copies the immutable artifact, independently recompiles the exact plan/profile/closure, re-verifies signed projection/PIT scope, and replays policy. The production reader opens every control file with no-follow/single-link/immutable checks; measures artifact digest, embedded manifest, Coverage proof, and logical snapshot identity through one descriptor; retains exact inode identity plus measured digest; rejects same-path replacement before or during SQLite descriptor transfer; and `VerifiedPilotReadyPublication` consumes the retained digest instead of rehashing a pathname. Hash/copy paths have fixed 64 GiB and cooperative deadline/free-space bounds. Passive registry/socket metadata remains truthfully `PENDING/UNKNOWN`; active keys=0. **OPERATIONAL-OPEN:** provision the READY UID/socket/key/registry/protected store, independently recheck the authenticated current mirror/closure/copy, and publish and verify one immutable exact-four READY; Mass stays disabled. |
...
| R11 | Controlled execution duplicated authority lineage into a caller-writable HOME store | OPEN | **SOURCE-REACHABLE (inactive):** `7871bd2c`; six-principal bootstrap/runner binds Controlled to its distinct UID/socket/raw Ed25519 seed/active registry/Trader UID/protected canonical store/immutable snapshot/signed projection activation. Runtime config has no mutable current DB capability, execution/promotion remain false, and active keys=0. **OPERATIONAL-OPEN:** provision the external writer UID/socket/fresh key/active registry/root-owned activation/protected canonical store, and accept one authorized Pilot artifact chain. |
