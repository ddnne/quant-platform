...
| R5 | Generic READY publication and a same-UID arbitrary READY signer remained reachable | OPEN | **SOURCE-CLOSED (inactive):** `ceaf9d21`, `e0fa86c4`, `52a8499d`; the product publisher exposes one client-only exact-four path and no importable production capability, injected production engine, signer or key fallback. Before signing, the dedicated authority descriptor-pins and copies the immutable artifact, independently recompiles the exact plan/profile/closure, re-verifies the signed projection and PIT natural-key/receipt-product scope, replays persisted Coverage plus raw/validation/natural-key/B0/B4/cursor policy query-only, exact-compares the rebuilt ReadyManifest, rechecks the active registry/artifact/deadline, and pins the exact attestation filename/digest/id/bytes in the marker. Passive registry/socket metadata remains truthfully `PENDING/UNKNOWN`; active keys=0. **OPERATIONAL-OPEN:** provision the READY UID/socket/key/registry/protected store, independently recheck the authenticated current mirror/closure/copy, and publish and verify one immutable exact-four READY; Mass stays disabled. |
