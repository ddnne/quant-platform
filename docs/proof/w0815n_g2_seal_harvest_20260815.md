# W21-G2 / w0815n_g2 — seal harvest window_ok unsealed (JQ, no DEFER densify) (2026-08-15)
...
**prefix:** `w0815n_g2_seal_harvest_*`  
...
**Live verified:** 2026-08-15 (JST) / ~2026-08-15T00:44Z UTC  
**Wave start HEAD:** `12d7592e096dcf510fa3b9dcdb3553528d6a4aa6`  
**Base HEAD (pre-proof):** `5f11b97128aa39a530ff5e54510ecb573dfc0c18`  
**Proof HEAD (post-push):** `d9c9f7dd4d884d9aeaf39da1a430553523ab3eca`  
**Projection:** **FRESH** `projgen-94809de8939844c69b07f1093557f8c3`  
**Artifacts:** `.glm-logs/w0815n_g2_seal_harvest/` (`scan_window_ok_fast.py`, `inventory_window_ok.json`, `harvest_verdict.json`, `PRE_remote.json`, `POST_remote.json`, `FINAL_metrics.json`, `publish.log`, `reeval_freshness.log`, `pages/`)

## Goal

1. Scan R2/local for **window_ok** PARTIAL months not COMPLETE across JQ (options already 164 — skip if done).
2. Seal → issue → restore any true harvest candidates (**nz raw** + **params window_ok** + **data Date in segment month**).
3. Optional tip densify only for **non-DEFER** tip holes.
...
PRE SHA (wave start): `12d7592e096dcf510fa3b9dcdb3553528d6a4aa6`  
Artifacts: `.glm-logs/w0815n_g2_seal_harvest/PRE_remote.json`, `PRE_sha.txt`, `PRE_partial_segments.json`
...
1. Cache-first index of `.glm-logs/**/manifests/<dataset>/*.json` (**6682** unique run files → **5486** parse_ok nz → **2824** best params-window_ok segments across care datasets).
...
Phase2 note: `markets_breakdown` R2 fetch reported `new_wok≈12` — all already **COMPLETE** months (not residual PARTIAL unsealed).
...
Artifacts:
- `.glm-logs/w0815n_g2_seal_harvest/inventory_window_ok.json`
- `.glm-logs/w0815n_g2_seal_harvest/seal_map_all_window_ok.json` (params-level; **not** seal input)
- `.glm-logs/w0815n_g2_seal_harvest/seal_map_non_defer.json` (**[]**)
- `.glm-logs/w0815n_g2_seal_harvest/harvest_verdict.json`
- `.glm-logs/w0815n_g2_seal_harvest/pages/` (data-Date gate samples)
...
ops_reeval_freshness
  OK gen=projgen-94809de8939844c69b07f1093557f8c3
...
| raw manifests | **15020** | **15037** | **+17** (peer worker pass ≠ COMPLETE) |
...
All remaining JQ PARTIAL residuals remain **DEFER-class** (D1–D7, D9 + fins pre-history empty shells). No new sealable window_ok island discovered vs W18-G2 harvest. Peer raw +17 during wave (worker pass only). Re-confirm: params-level false positives (earn **199** + master **20**) still fail data-Date gate.

## Summary line

`seal_harvest=0 (params_wok_false_positive earn199+master20 data-Date REJECT) | densify_skip DEFER+no_tip_holes | options 164 skip | COMPLETE 3442→3442 | raw 15020→15037 | Dataset COMPLETE 11 | FRESH projgen-94809de… | empty 0 | DEFER densify ban held | push`
