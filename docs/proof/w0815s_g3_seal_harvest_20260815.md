# W26-G3 / w0815s_g3 — seal harvest with data-Date gate (2026-08-15)
...
**Live verified:** 2026-08-15 (JST) / ~2026-08-15T02:26Z UTC  
**Wave start HEAD:** `1dd7ddd6ea9fad1b4dd99ec10d64a08e26773fb7`  
**Base HEAD (pre-proof):** `1dd7ddd6ea9fad1b4dd99ec10d64a08e26773fb7`  
**Proof HEAD (post-push):** `b3c50937c3463a43c2612d8e49654991dd80b5a8`  
**Projection:** **FRESH** `projgen-df7978dbcd3d4c9fb587ee6e4218c634`  
**Artifacts:** `.glm-logs/w0815s_g3_seal/` (`scan_window_ok_fast.py`, `data_date_gate.py`, `inventory_window_ok.json`, `harvest_verdict.json`, `PRE_remote.json`, `POST_remote.json`, `FINAL_metrics.json`, `publish.log`, `reeval_freshness.log`, `pages/`)
...
| `fins_summary` | **218** |
| `jsda_otc_bond_reference_prices` | **72** (peer tip; D5 archive still DEFER) |
| local COMPLETE | **3457** (= remote) |

PRE SHA (wave start): `1dd7ddd6ea9fad1b4dd99ec10d64a08e26773fb7`  
Artifacts: `.glm-logs/w0815s_g3_seal/PRE_remote.json`, `PRE_sha.txt`, `PRE_partial_segments.json`
...
1. Cache-first index of `.glm-logs/**/manifests/<dataset>/*.json` (**6794** unique run files → **5574** parse_ok nz → **2825** best params-window_ok segments across care datasets).
...
Phase2 note: all residual PARTIAL families returned **new_wok=0** for unsealed residual months (any tip-month window_ok hits already COMPLETE).
...
Artifacts:
- `.glm-logs/w0815s_g3_seal/inventory_window_ok.json`
- `.glm-logs/w0815s_g3_seal/seal_map_all_window_ok.json` (params-level; **not** seal input)
- `.glm-logs/w0815s_g3_seal/seal_map_non_defer.json` (**[]**)
- `.glm-logs/w0815s_g3_seal/harvest_verdict.json`
- `.glm-logs/w0815s_g3_seal/pages/` (data-Date gate samples)
...
ops_reeval_freshness
  OK gen=projgen-df7978dbcd3d4c9fb587ee6e4218c634
...
| raw manifests | **15125** | **15142** | **+17** (peer worker pass ≠ COMPLETE) |
| options segs | **164** | **164** | 0 |
| OTC COMPLETE | **72** | **72** | 0 |
| empty COMPLETE (this wave) | **0** | **0** | held |
| G3 owned seal/issue | — | **0** | honest +0 |

## Residual SoT note

All remaining JQ PARTIAL residuals remain **DEFER-class** (D1–D7, D9 + fins pre-history empty shells). No new sealable window_ok island discovered vs W23-G3 harvest. Peer raw +17 during wave (worker pass only). Re-confirm: params-level false positives (earn **199** + master **20**) still fail data-Date gate (tip `2026-08-14` / master shell `2008-05-07`).

Baseline note vs prior wave: remote COMPLETE held **3457** (W23-G3 POST); G3 harvest delta remains **0**.

## Summary line

`seal_harvest=0 (params_wok_false_positive earn199+master20 data-Date REJECT) | densify_skip DEFER+no_tip_holes | options 164 skip | COMPLETE 3457→3457 | raw 15125→15142 | Dataset COMPLETE 11 | FRESH projgen-df7978dbcd3d… | empty 0 | DEFER densify ban held | push`
