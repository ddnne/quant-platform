# W23-G3 / w0815p_g3 — seal harvest with data-Date gate (2026-08-15)
...
PRE SHA (wave start): `541a33f8f4c9082c633e7d301ccd76394838a2c2`  
Artifacts: `.glm-logs/w0815p_g3_seal/PRE_remote.json`, `PRE_sha.txt`, `PRE_partial_segments.json`
...
1. Cache-first index of `.glm-logs/**/manifests/<dataset>/*.json` (**6770** unique run files → **5562** parse_ok nz → **2825** best params-window_ok segments across care datasets).
...
Artifacts:
- `.glm-logs/w0815p_g3_seal/inventory_window_ok.json`
- `.glm-logs/w0815p_g3_seal/seal_map_all_window_ok.json` (params-level; **not** seal input)
- `.glm-logs/w0815p_g3_seal/seal_map_non_defer.json` (**[]**)
- `.glm-logs/w0815p_g3_seal/harvest_verdict.json`
- `.glm-logs/w0815p_g3_seal/pages/` (data-Date gate samples)
...
ops_reeval_freshness
  OK gen=projgen-bafc1270a7b448c798245014f61749cf
...
| raw manifests | **15097** | **15100** | **+3** (peer worker pass ≠ COMPLETE) |
...
All remaining JQ PARTIAL residuals remain **DEFER-class** (D1–D7, D9 + fins pre-history empty shells). No new sealable window_ok island discovered vs W22-G3 harvest. Peer raw +3 during wave (worker pass only). Re-confirm: params-level false positives (earn **199** + master **20**) still fail data-Date gate (tip `2026-08-14` / master shell `2008-05-07`).

Baseline note vs prior wave: remote COMPLETE **3442→3457** before this wave (peer OTC/tip seals outside G3); G3 harvest delta remains **0**.

## Summary line

`seal_harvest=0 (params_wok_false_positive earn199+master20 data-Date REJECT) | densify_skip DEFER+no_tip_holes | options 164 skip | COMPLETE 3457→3457 | raw 15097→15100 | Dataset COMPLETE 11 | FRESH projgen-bafc1270… | empty 0 | DEFER densify ban held | push`
