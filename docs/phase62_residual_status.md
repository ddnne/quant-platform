...
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v138-24do-plus22` (`cce6b7e3-3d1c-497c-bc2e-2bf89ddc3c8a`). Baskets keep `eval-cf-dp-both-sleeves-20260824df`. Nested detect + reconstitution_options; **apply false** after stitched HONEST-3y blend `eval-reconstitution-blend-20260824dz`. Sleeve defs + primary members cached. Historical sleeves `deprecated` (still valid). Cost/risk `eval-usable-cost-risk-20260824eb` not_a_pass. Usable `eval-usable-inventory-20260824eb` n_usable 1724. event 3-AND +N **stopped**. Combo jsonl `eval-combo-jsonl-20260824eb` yaml_remains_sot. Deleted 12 uncited w0713/bars proofs. Propose `eval-cf-propose-20260824eb` llm_failed 0-adopt (not soup). unique22 5/17. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`.
