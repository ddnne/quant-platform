...
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v140-24eu-vol50` (`445267c8-ac66-43e2-91d1-6cdb7b02e307`). Baskets keep `eval-cf-dp-both-sleeves-20260824df`; alts `eval-cf-dp-basket-alts-20260824eu` apply false (vol alts occupancy-thinner than keep flow). Nested detect + reconstitution_options; **apply false** (`RECONSTITUTION_APPLY` false; human pending fund/event_fund) after stitched HONEST-3y blend `eval-reconstitution-blend-20260824dz`. Historical sleeves are not `candidate` in basket-trend summary. Cost/risk `eval-usable-cost-risk-20260824eu` not_a_pass. Usable `eval-usable-inventory-20260824eu` n_usable 1830; series `eval-usable-series-20260824eu`; occupancy maps `eval-occupancy-maps-20260824eu`; occupancy audit `eval-occupancy-audit-20260824eu-vol50` vol-thin +50 material thin 0 near_empty 0 (not 3-AND). Thin-series `eval-thin-series-20260824eu` vol then flow. Drift `eval-occupancy-drift-20260824eu` 0 silent-unpark. unique22 park `eval-unique22-park-20260824eu` 5/17. event 3-AND +N **stopped**. Combo jsonl `eval-combo-jsonl-20260824eu` yaml_remains_sot. Flow 5th stitch blend `eval-flow-5th-blend-20260824ek` thinner; keep current. Reconstitution `eval-reconstitution-plan-20260824eu` apply false. Series-sleeve `eval-series-sleeve-20260824eu`. Propose `eval-cf-propose-20260824eu` llm_failed 0-adopt (not soup). Wave one-call `research.occupancy_audit.run_eval_wave`. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`. Deleted `scripts/record_research_eval.py`. `research.cost_defaults` is the shared 10bp literal.
