...
- **mid_n_explore** (ADV 80): `eval-occupancy-audit-20260824di-b0-mid_n_explore`, `eval-cf-dp-both-sleeves-20260824df-mid_n_explore`
- **liq_large** (ADV 100): `eval-occupancy-audit-20260824di-b0-liq_large`, `eval-cf-dp-both-sleeves-20260824df-liq_large`
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v132-24di-plus48` (`9f59f6d8-4e75-4af7-b22a-1e65fb719cd4`). Baskets `eval-cf-dp-both-sleeves-20260824df` not_a_pass (`eval-basket-decision-20260824di` keep: new lo max 0.335 < flow 4th 0.467). Durability `eval-usable-read-20260824di` not_a_pass. Usable `eval-usable-inventory-20260824di` n_usable 1390 (was 1342). near_empty park 63. Thin sleeve exclude 213. always_on park 7. 24di +48 surprise_xs 3-AND (48 material / 0 thin / 0 empty; non-event 100%; parent lo>0.22). Deleted code: `run_catalog.py`, `rebuild_paper_index.py`, `run_agents_paper_once.py`, `tests/test_smoke.py` plus 12 uncited proof logs. Propose `eval-cf-propose-20260824di` n_adoptable 1 `afterclose+price_down+easy_funding` not written (parent lo 0.222 below yield filter; 48 already occupancy-correct). unique22 5/17. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`.
