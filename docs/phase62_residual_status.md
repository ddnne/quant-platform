...
Both-track packs `research/eval/job=eval-cf-dp-both-sleeves-20260822c/both_track.json` and `research/eval/job=eval-cf-dp-both-sleeves-20260822m/both_track.json` (reblend). Cost ON/OFF `research/eval/job=eval-cf-dp-cost-onoff-20260822c/cost_verify.json`. Cost short/high-turnover `research/eval/job=eval-cf-dp-cost-shortturn-20260822h/cost_verify.json`. Composition compare `research/eval/sleeve_stability/headn100_vs_liq100b.json`. Triad `research/eval/sleeve_stability/univ50_vs_univ80_vs_univ100.json`. Universe rule `adv_desc_skip_missing_bars_and_fins` (head-N forbidden on both tracks). Isolate park set is empty after `csFundSnaps` hoist + `eval-cf-dp-cs-hoist-20260822a` (N=100 complete). Linearized: cluster three + those three CS. Worker `research-mass-eval/v27-propose-reason` (LLM fallback reason logged; silent stub-as-success forbidden). Head-N list slice forbidden on both tracks. GO deferred. Thesis count requires catalog + Worker body (`countable_thesis_ids`). unique-22 re-audit: still 5 occupancy-equal lifts / 17 `unique22_occupancy_mismatch` (no extra lift). Event `cheap_pb` is bars×fins; CS `cheap_pb` is csFundSnaps (`CHEAP_PB_UNIFIED=false`). CF propose `POST /v1/propose-thesis` (2–3 economic AND; `llm_fallback_reason`). Latest propose: k stub (`n_adoptable=0`); m Workers AI used, clone reject, `n_adoptable=0`. plus32 jobs `eval-cf-dp-liq100-plus32-20260822m` / `eval-cf-dp-mid80-plus32-20260822m`. Primary baskets reblended from countable (weekday members off DEFAULT_CANDIDATE_BASKET).
