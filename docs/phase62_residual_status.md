...
- **mid_n_explore** (ADV 80): `eval-occupancy-audit-20260824z-mid_n_explore`, `eval-cf-dp-both-sleeves-20260824e-mid_n_explore`
- **liq_large** (ADV 100): `eval-occupancy-audit-20260824z-liq_large`, `eval-cf-dp-both-sleeves-20260824e-liq_large`
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v50-24z-adopt` (`3afa8fc7-c072-4d86-bca9-a63427ed281f`). Baskets `eval-cf-dp-both-sleeves-20260824e` not_a_pass. Durability `eval-durability-summary-20260823r` not_a_pass. Parked-empty YAML must have a SPARSE_GATE_COMBOS subset. near_empty park 14. Thin sleeve exclude. Occupancy `eval-occupancy-audit-20260824z`. Inventory `eval-inventory-bias-20260824z`. Propose 24z: first clone 0-adopt then retry 2-adopt (steep×tight×px, easing×eps×np) both-track thin, not sleeve (event_fund at 5). Steep occupancy-label + eps_down positive-earnings contra. GOOD stays p10×np. unique22 5/17. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`.
