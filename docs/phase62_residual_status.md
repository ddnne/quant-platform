...
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v139-24em-plus56` (`444f25f3-1e42-4b94-806a-df3e7b76d915`). Baskets keep `eval-cf-dp-both-sleeves-20260824df`; alts `eval-cf-dp-basket-alts-20260824em` apply false. Nested detect + reconstitution_options; **apply false** after stitched HONEST-3y blend `eval-reconstitution-blend-20260824dz`. Historical sleeves are not `candidate` in basket-trend summary. Cost/risk `eval-usable-cost-risk-20260824es` not_a_pass. Usable `eval-usable-inventory-20260824es` n_usable 1780; series `eval-usable-series-20260824es`; occupancy maps `eval-occupancy-maps-20260824es`; occupancy audit `eval-occupancy-audit-20260824em-plus56` 2-AND vol/flow/rate/fund +56 material thin 0 near_empty 0. Drift `eval-occupancy-drift-20260824es` 0 silent-unpark. unique22 park `eval-unique22-park-20260824es` 5/17. event 3-AND +N **stopped**. Combo jsonl `eval-combo-jsonl-20260824es` yaml_remains_sot. Flow 5th stitch blend `eval-flow-5th-blend-20260824ek` thinner; keep current. Reconstitution `eval-reconstitution-plan-20260824es` apply false (fund nested 1, event_fund nested 3); occupancy_preview v1 is map occupancy not a blend. Series-sleeve `eval-series-sleeve-20260824es` invert_primary false; `BLEND_THINNER_KEEP_IDS` excludes afterclose_uncrowded / peps_uncr. Propose `eval-cf-propose-20260824es` review-ok 3-AND n_pri<2 **not written** (not soup). Wave one-call `research.occupancy_audit.run_eval_wave`. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`. Deleted `packages/product/research/evaluation.py` and `tests/test_phase622_remainder.py`. `research.cost_defaults` is the shared 10bp literal.
