...
- **liq_large** (ADV 100): `eval-cf-dp-liq100-20260822b`, `eval-cf-dp-liq100-cross-20260822a`, `eval-cf-dp-liq100-sleeves-20260822a`, `eval-cf-dp-baskets-liq100-sleeves-20260822a`, `eval-cf-dp-both-sleeves-20260822c-liq_large`, `eval-cf-dp-liq100-new22-20260822c`, `eval-cf-dp-liq100-plus4-20260822d`, `eval-cf-dp-liq100-plus28-20260822h`, `eval-cf-dp-liq100-adopt3-20260822i`, `eval-cf-dp-liq100-adopt3j-20260822j` (head-N contrast `eval-cf-dp-univ100-20260822a` / `eval-cf-dp-baskets100-20260822a`)
Both-track pack `research/eval/job=eval-cf-dp-both-sleeves-20260822c/both_track.json` (registry put; isolate `write_artifacts` first honored on later SHA). Cost ON/OFF `research/eval/job=eval-cf-dp-cost-onoff-20260822c/cost_verify.json`. Cost short/high-turnover `research/eval/job=eval-cf-dp-cost-shortturn-20260822h/cost_verify.json`. Composition compare `research/eval/sleeve_stability/headn100_vs_liq100b.json`. Triad `research/eval/sleeve_stability/univ50_vs_univ80_vs_univ100.json`. Universe rule `adv_desc_skip_missing_bars_and_fins` (head-N forbidden on both tracks). Isolate park set is empty after `csFundSnaps` hoist + `eval-cf-dp-cs-hoist-20260822a` (N=100 complete). Linearized: cluster three + those three CS. Worker `research-mass-eval/v26-propose-and3` (propose 2–3 gate AND; 4-gate adopt3 occupancy mostly near_empty, not a candidate). Head-N list slice forbidden on both tracks. GO deferred. Thesis count requires catalog + Worker body (`countable_thesis_ids`). unique-22: 5 occupancy-equal lifts send `params.gates`; 17 parked `unique22_occupancy_mismatch`. Event `cheap_pb` is bars×fins; CS `cheap_pb` is csFundSnaps (`CHEAP_PB_UNIFIED=false`). CF propose is `POST /v1/propose-thesis` (Workers AI bound, stub fallback, no auto-inject; 2+ economic gates AND-cross; no prompt-direction echo; no contradictory pairs). Latest propose jobs `eval-cf-propose-llm-20260822e`–`h` (not injected), `eval-cf-propose-llm-20260822i` (4-gate; occupancy mostly near_empty; not a candidate), `eval-cf-propose-llm-20260822j` (`n_adoptable=3` 2–3 gate AND; reviewed adopt `event_eqar_rising_ta_down_tight` / `event_eps_up_margin_up_easing` / `event_np_negative_price_down_liq_fade`), `eval-cf-propose-llm-20260822k` (Workers AI fell back to stub; `n_adoptable=0`; no YAML). Review sidecar `research/eval/job=eval-cf-propose-llm-20260822k/review.json`. why_avoid is economic 2-gate catalog sets (no weekday). `scripts/sync_cf_new_thesis_ids.py --check` pins Worker allowlists. unique22 park YAML comments on 17 leftover IDs.
