...
Universe `adv_desc_skip_missing_bars_and_fins` (head-N forbidden). Worker `research-mass-eval/v139-24em-plus56` (`444f25f3-1e42-4b94-806a-df3e7b76d915`). Baskets keep `eval-cf-dp-both-sleeves-20260824df`; alts `eval-cf-dp-basket-alts-20260824em` apply false. Nested detect + reconstitution_options; **apply false** (`RECONSTITUTION_APPLY` false; human pending fund/event_fund) after stitched HONEST-3y blend `eval-reconstitution-blend-20260824dz`. Historical sleeves are not `candidate` in basket-trend summary. Cost/risk `eval-usable-cost-risk-20260824et` not_a_pass. Usable `eval-usable-inventory-20260824et` n_usable 1780; series `eval-usable-series-20260824et`; occupancy maps `eval-occupancy-maps-20260824et`; occupancy audit `eval-occupancy-audit-20260824em-plus56` 2-AND vol/flow/rate/fund +56 material thin 0 near_empty 0. Drift `eval-occupancy-drift-20260824et` 0 silent-unpark. unique22 park `eval-unique22-park-20260824et` 5/17. event 3-AND +N **stopped**. Combo jsonl `eval-combo-jsonl-20260824et` yaml_remains_sot. Flow 5th stitch blend `eval-flow-5th-blend-20260824ek` thinner; keep current. Reconstitution `eval-reconstitution-plan-20260824et` apply false; occupancy_preview human_choice_required. Series-sleeve `eval-series-sleeve-20260824et` n_replacement_ok 0 (weakest-lo + 24ek lock). Propose `eval-cf-propose-20260824et` llm_failed 0-adopt (not soup); `PROPOSE_BLOCKED_GATE_SETS` keeps 24es n_pri<2 3-AND off why_avoid. Wave one-call `research.occupancy_audit.run_eval_wave`. GO deferred. HOLD: cost_models / options_225 / daily_path leftover / unique22 park YAML / CLI `__main__.py` / `ingestion.jsda.adapters` / `jquants/bulk.py`. Deleted `packages/product/research/evaluation.py` and `tests/test_phase622_remainder.py`. `research.cost_defaults` is the shared 10bp literal.
