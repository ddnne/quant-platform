...
- **liq_large** (ADV 100): `eval-cf-dp-liq100-20260822b`, `eval-cf-dp-liq100-cross-20260822a`, `eval-cf-dp-liq100-sleeves-20260822a`, `eval-cf-dp-baskets-liq100-sleeves-20260822a`, `eval-cf-dp-both-sleeves-20260822c-liq_large`, `eval-cf-dp-liq100-new22-20260822c`, `eval-cf-dp-liq100-plus4-20260822d`, `eval-cf-dp-liq100-plus28-20260822h`, `eval-cf-dp-liq100-adopt3-20260822i`, `eval-cf-dp-liq100-adopt3j-20260822j`, `eval-cf-dp-liq100-plus32-20260822m`, `eval-cf-dp-both-sleeves-20260822m-liq_large`, `eval-cf-dp-liq100-adopt3n-20260822n`, `eval-cf-dp-both-sleeves-20260822p-liq_large`, `eval-cf-dp-liq100-adopt1q-20260822q` (head-N contrast `eval-cf-dp-univ100-20260822a` / `eval-cf-dp-baskets100-20260822a`)
Both-track packs `research/eval/job=eval-cf-dp-both-sleeves-20260822c/both_track.json`, `research/eval/job=eval-cf-dp-both-sleeves-20260822m/both_track.json` (reblend), and `research/eval/job=eval-cf-dp-both-sleeves-20260822p/both_track.json` (`n_logic_ok` on track rows). Cost ON/OFF `research/eval/job=eval-cf-dp-cost-onoff-20260822c/cost_verify.json`. Cost short/high-turnover `research/eval/job=eval-cf-dp-cost-shortturn-20260822h/cost_verify.json`. Composition compare `research/eval/sleeve_stability/headn100_vs_liq100b.json`. Triad `research/eval/sleeve_stability/univ50_vs_univ80_vs_univ100.json`. Universe rule `adv_desc_skip_missing_bars_and_fins` (head-N forbidden on both tracks). Isolate park set is empty after `csFundSnaps` hoist + `eval-cf-dp-cs-hoist-20260822a` (N=100 complete). Linearized: cluster three + those three CS. Worker `research-mass-eval/v28-title-polarity` (title polarity + occupancy sentence + clone-retry extra prepended; silent stub-as-success forbidden). Head-N list slice forbidden on both tracks. GO deferred. Thesis count requires catalog + Worker body (`countable_thesis_ids`; test floor `>= 30`). unique-22 re-audit: still 5 occupancy-equal lifts / 17 `unique22_occupancy_mismatch` (no extra lift). Event `cheap_pb` is bars×fins; CS `cheap_pb` is csFundSnaps (`CHEAP_PB_UNIFIED=false`). CF propose `POST /v1/propose-thesis` (2–3 economic AND; `llm_fallback_reason`; `title_gate_polarity_mismatch`; `title_not_occupancy`). Latest propose: k stub (`n_adoptable=0`); m Workers AI clone reject; n Workers AI `n_adoptable=3` (auth token no longer overwritten by why_avoid; clone retry armed); p Workers AI `n_adoptable=2` ×-label mashups **not adopted**; q Workers AI occupancy sentence `n_adoptable=1`. Reviewed adopt `event_eqar_rising_cheap_pb_liq` / `event_sales_down_tight_fade` / `event_ta_up_np_neg_price_down_fade` / `event_margin_down_tight_steep_curve`. plus32 jobs `eval-cf-dp-liq100-plus32-20260822m` / `eval-cf-dp-mid80-plus32-20260822m`. Mechanical weekday baskets retired (`event_calendar_only` dict dropped from `MECHANICAL_BASKETS`) or reblended to countable economic members. unique22 park 17 unchanged. Dead-code this turn (file list): deleted `tests/test_research_offline_factory.py`; dropped retired `basket_event_cal4` members dict; catalog integer count-pins (`==307/411/433`) replaced by YAML-derived equality. Not deleted: `cost_models.py` / `options_225_vol_series.py` / `daily_path.ts` leftover occupancy / factory `generation_enabled=False` / `paper_candidate_adapter.py` barrel / unique22 leftover.
