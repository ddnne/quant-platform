# Phase 6.3.2 P — test inventory at `67fcbd7c`

**Kind:** read-only inventory. Does **not** delete tests. Does **not** invent
`tests_after`. Does **not** flip GO.  
**SHA counted:** `67fcbd7cd56847a9fc0fba7bcefbd743b43fc106` (`67fcbd7c`)  
`docs: independent review C catalog/pilot revisit at 40d1aa90`  
**Isolation worktree:** `/private/tmp/qp-p632-test-inventory-67fcbd7c` on
`docs/p632-test-inventory-67fcbd7c` off `origin/grok/phase63-ci-source-closure`.  
...
| Surface | How counted (this isolation tree at `67fcbd7c`) |
|---------|-------------------------------------------------|
| Python collected | `/Users/taku/GitHub/quant-platform/.venv/bin/python -m pytest --collect-only -q tests/` from this worktree. Python 3.11.15, pytest 9.1.1 (`pyproject.toml` `addopts = "-q"`). **PASS**, `1448 tests collected in 0.79s`. |
...
| Metric | Brief §P (`tests_before`) | `40d1aa90` inventory | This SHA (`67fcbd7c`) |
|--------|--------------------------:|---------------------:|----------------------:|
| **tests_before (collected)** | ~**1282** | — | — |
| **tests_after (collected)** | *not invented* | *not invented* | *not invented* |
| **collected now (actual)** | — | **1426** | **1448** |
| `tests/test_*.py` files | — | **146** | **148** |
| Worker `*.test.ts` | — | **16** | **16** |
| Worker `*.test.mjs` | — | **4** | **4** |
| Worker first-party test files | — | **20** | **20** |
| Worker source `it(` / `test(` | — | — | **141** (not vitest collect) |
| `specs/research_logics` YAML | — | **0** | **0** (dir exists; only `README.md`) |

`1448 > 1426 > 1379 > 1282`. Later landings grew the suite vs `40d1aa90`
(new modules: `test_refresh_coverage_ledger_cli.py`,
`test_write_collection_receipts.py`; existing modules also grew). That is
**not** a consolidation win and **not** a GO. YAML under
...
- Collect-only **succeeded**. Collected **1448**. Brief `tests_before` ~**1282**.
  Prior inventory **1426** at `40d1aa90`. `tests_after` is **not invented**.
  The +22 vs `40d1aa90` is not a win.
...
This file is a now-inventory receipt at `67fcbd7c`, not a pass.
