# Independent review B revisit — at `3b64bdfc`

**Reviewer:** independent Grok (isolation worktree; not the implementer)  
**Reviewed HEAD:** `3b64bdfc` (`3b64bdfc9a41be76a6e4e881aaea1ff9751443ed`)  
**Branch at audit:** `grok/p632-ind-B-revisit-3b64bdfc` (from `grok/phase63-ci-source-closure`)  
**Prior revisits:** `5103b26b` ([`P632_ind_B_revisit_5103b26b.md`](P632_ind_B_revisit_5103b26b.md)); `ed94d504` ([`P632_ind_B_revisit_ed94d504.md`](P632_ind_B_revisit_ed94d504.md)); `67fcbd7c` ([`P632_ind_B_revisit_67fcbd7c.md`](P632_ind_B_revisit_67fcbd7c.md)); `40d1aa90` ([`P632_ind_B_revisit_40d1aa90.md`](P632_ind_B_revisit_40d1aa90.md)); `f224e7e` ([`P632_ind_B_revisit_f224e7e.md`](P632_ind_B_revisit_f224e7e.md)); `07b4435` ([`P632_ind_B_revisit.md`](P632_ind_B_revisit.md)); freeze `a48c600` ([`P632_ind_B_ci_authority.md`](P632_ind_B_ci_authority.md))  
**origin/main:** `b5c326a7f612563f2da4a84f08063a307ec38e0a` (feature HEAD is **not** an ancestor of `main`; not merged)  
**Scope:** re-measure Independent B — CI skip, `ci-aggregate` live producer, tokens, R2, budget — vs HEAD `3b64bdfc`. Tree-level receipt auth and seven-worker `verify_ci` already landed before `07b4435`. This turn re-diffs those plus P632B-03..07 against `5103b26b`. Live GitHub check-runs, `main` protection, and `wrangler deployments list --name quant-platform-ci-aggregate` are this-turn measurements. P632B-05 is the code-lane delta: overlay CLI put is gone; several job-artifact callers now `put_research_artifact`.
...
| `projection_age_seconds` | 185842 (~51.62 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `5103b26b`. Empty diff on `platform/workers/ci-aggregate`. |
| Does `verify_ci` include `ci-aggregate` (7 workers)? | **Yes.** `WORKERS` has **7** paths; `tests/test_verify_ci_script.py` pins `len(WORKERS) == 7` and `for name in WORKERS: assert name in src`. Script body is an empty diff vs `5103b26b`. Test pins now require the freeze calls to be invoked (`assert_…()`). |
| Are GitHub check-runs still 0 live at `3b64bdfc`? | **Yes.** HEAD `3b64bdfc`, prior `5103b26b`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = []`. Actions workflows **0**. |
...
## Scoreboard vs `5103b26b` / `ed94d504` / `67fcbd7c` / `40d1aa90` / `f224e7e` / `07b4435` / `a48c600`

| ID | Topic | Sev | At `a48c600` | At `07b4435` | At `f224e7e` | At `40d1aa90` | At `67fcbd7c` | At `ed94d504` | At `5103b26b` | At `3b64bdfc` |
|----|-------|-----|--------------|--------------|--------------|---------------|---------------|---------------|---------------|---------------|
| P632B-01 | `verify_all` skip vs `verify_ci` authority | P0 | OPEN (scripts split FIXED; `ci-aggregate` absent from `verify_ci`; merge gate not `verify_ci`) | OPEN (`ci-aggregate` in `verify_ci` FIXED; GitHub still does not run `verify_ci`) | OPEN (IR jsonschema + `wrangler types --check` in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`npm run types -- --check` + ALLOWED_FIELDS freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (seven-worker `verify_ci` unchanged vs `40d1aa90`; merge gate still six lane receipts) | OPEN (IR encode-keys freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (IR codec-generated freeze in `verify_ci` FIXED; merge gate still six lane receipts) | **OPEN** (`verify_ci.sh` empty vs `5103b26b`; test pins now require freeze **calls**; merge gate still six lane receipts) |
...
| P632B-03 | Shared `GATEWAY_TOKEN` / `MASS_EVAL_TOKEN` | P1 | OPEN (header mix-up FIXED) | not re-diffed | OPEN (mass-eval still sends `GATEWAY_TOKEN`) | OPEN (mass-eval still sends `GATEWAY_TOKEN`) | OPEN (empty diff vs `40d1aa90`: mass-eval still sends `GATEWAY_TOKEN`) | OPEN (empty diff vs `67fcbd7c`: mass-eval still sends `GATEWAY_TOKEN`) | OPEN (empty diff vs `ed94d504`: mass-eval still sends `GATEWAY_TOKEN`) | **OPEN** (empty diff vs `5103b26b`: mass-eval still sends `GATEWAY_TOKEN`) |
...
| P632B-05 | R2 child 409 / Python TOCTOU writer | P0/P1 | FIXED Worker 409; OPEN Python writer | not re-diffed | same | same severity (Worker `onlyIf` FIXED; remote Python put fail-closed without `QP_ALLOW_PYTHON_R2_PUT=1`) | same severity (Worker `onlyIf` still FIXED; `61c14a0d` Worker-client entry fail-closes with no HTTP client and no CLI fallback; callers still `default_r2_put`) | same severity (empty diff vs `67fcbd7c` on Worker `http.ts` and `r2_io.py`) | Worker 409 FIXED; Python default path POST; opt-in `QP_ALLOW_PYTHON_R2_PUT` still TOCTOU (P1 still OPEN; existing `default_r2_put` callers remain) | **Worker 409 FIXED; overlay CLI TOCTOU CLOSED in tree; leftover `default_r2_put` callers remain (fail-closed, not racing)** (P1 residual OPEN; not live R2 proof) |
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | same | same (no `catalog_kind` in mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | **same** (empty grep for `catalog_kind` under mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | FIXED | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |

Independent P0 unresolved: **2**. Tree-level holes named at freeze for receipt auth and seventh-worker `verify_ci` stay closed. Overlay CLI put on `default_r2_put` is closed in tree. The live merge gate still has **no honest producer**. Do not invent Coverage COMPLETE, Projection FRESH, B0 PASS, READY, or CI green.

Window vs `5103b26b`: `git rev-list --count 5103b26b..3b64bdfc` = **16**. CI-surface deltas that are **not** a P0 close: `2e264a08` (`verify_ci` test pins require freeze **calls** `assert_…()`); `a4453658` (docs name print-only first-deploy helper); `52f3e70e` (mass-eval children-then-manifest unbound token → 503); `0b81eedb` (`default_r2_put` never CLI-puts even with `QP_ALLOW_PYTHON_R2_PUT=1`); `d6567268` (job-artifact callers `put_research_artifact`). `platform/workers/ci-aggregate`, `scripts/verify_ci.sh`, `scripts/ci_aggregate_first_deploy.sh`, and `platform/workers/research-ai-gateway` are **empty diffs** against `5103b26b`. Tokens / budget / catalog / GHA surfaces other than the R2 caller move are **empty diffs** against `5103b26b`. Worker `http.ts` `onlyIf` is an **empty diff** against `5103b26b`. Print-only `scripts/ci_aggregate_first_deploy.sh` is **not** a producer.
...
`scripts/verify_ci.sh` still lists seven Workers (empty `WORKERS` array vs `5103b26b`):
...
Delta vs `5103b26b` (`2e264a08`): `tests/test_verify_ci_script.py` now asserts the freeze names **with call parens** (`assert_evaluation_ir_allowed_fields_ts_frozen()`, `assert_evaluation_ir_codec_ts_frozen()`, `assert_evaluation_ir_encode_keys_match_schema()`). Script body is unchanged. Local CI is the same authority. GitHub still does not run `verify_ci`.
...
`tests/README.md` names `scripts/verify_ci.sh` as mandatory local CI; `verify_all.sh` is the helper. `scripts/README.md` leads with `verify_ci.sh` (seven workers including `ci-aggregate`). `docs/ci/workers_builds.md:96-102` still says six-lane `npm test` receipts “skip Python/catalog and are **not** `verify_ci`”. Merge “requires GitHub context `ci-aggregate` after authenticated receipts **and** `verify_ci`”. Docs delta vs `5103b26b` is two lines naming the print-only first-deploy helper (`a4453658`); that is not a merge-gate close.

This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_5103b26b.md`](P632_verify_ci_5103b26b.md) is a later local PASS at `5103b26b`, not a GitHub status and not this SHA.
...
PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `3b64bdfc`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “`ci-aggregate` is in `verify_ci.sh`” or “codec freeze calls are pinned” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `5103b26b` on `platform/workers/ci-aggregate`:
...
`docs/ci/workers_builds.md:19` still says: “live: Worker absent from account as of Independent B f224e7e/40d1aa90 — HUMAN create.” This turn re-measures: still absent. Line 21 now names the print-only helper.

`scripts/ci_aggregate_first_deploy.sh` is an **empty diff** vs `5103b26b`. Default is dry-run / print-only. `--apply` without `CONFIRM_CI_AGGREGATE_CREATE=1` fails closed. With the confirm env, the script **still prints** `npx wrangler deploy` as a comment and does **not** exec it. Tests pin no live secret values. That helper is operator text, not a producer.

### Live GitHub / Cloudflare (this isolation turn)

Fetched 2026-08-23T16:11Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
PR #1  head=3b64bdfc  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=[]
```
...
1. has never been posted on `origin/main`, this reviewed SHA `3b64bdfc`, prior revisit `5103b26b`, or earlier Independent B SHAs,
...
A print-only first-deploy helper does not create the Worker. Do not treat `7dbcd9ea` / `a4453658` as P632B-02 FIXED.
...
**status:** **OPEN** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `5103b26b`.
...
Mass-eval HTTP routes authorize with `env.MASS_EVAL_TOKEN` / `X-Mass-Eval-Token` only. Unbound `MASS_EVAL_TOKEN` on `POST /v1/children-then-manifest` is now **503** (`52f3e70e`) — that is inbound mass-eval auth, not a gateway-token split.
...
**status:** mixed. Production budget path empty vs `5103b26b`. `046ae438` is leftover unique-22 occupancy HOLD in `unique_logic/worker_bodies.py`, not the gateway ledger.
...
`89415105` still pins: created ledger has **zero** occupancy; `budget_id` presence is not a reserve. Comment: “Live Cloudflare Durable Object occupancy is unproven.” That is honesty, not a live occupancy proof and not a P1 close. Empty production-path diff vs `5103b26b`.
...
**severity:** P0 Worker overwrite **FIXED**; overlay CLI TOCTOU **FIXED in tree**; P1 leftover callers **OPEN**  
...
**status:** mixed. Worker `onlyIf` empty vs `5103b26b`. Overlay CLI put is closed (`0b81eedb`). Job-artifact default path is `put_research_artifact` (`d6567268`). Remaining `default_r2_put` callers fail closed on remote (not racing). Live R2 POST unproven.

### FIXED (Worker 409)

`putJsonCreateOnly` (`http.ts:113-138`): `onlyIf: { etagDoesNotMatch: "*" }`; existing key with a different digest → 409. Child conflict still does not mint a manifest. Empty diff vs `5103b26b` on `http.ts`.
...
At `5103b26b`, `put_children_then_manifest_via_worker` POSTed `/v1/children-then-manifest`, but remote `default_r2_put` still CLI-put when `QP_ALLOW_PYTHON_R2_PUT=1`. This HEAD:

- `default_r2_put` remote path **always** raises `WORKER_CHILDREN_THEN_MANIFEST_ERROR` (`r2_io.py:361`). Overlay `=1` does **not** resurrect `wrangler r2 object put`. Tests pin `subprocess.run` is not called with overlay (`test_remote_r2_put_does_not_cli_put_with_overlay`). Source pin: `default_r2_put` has no `subprocess`.
...
- Job-artifact callers now use `put_research_artifact`: `cf_mass_eval_job`, `cf_mass_eval_stage`, `cf_cost_verify`, `cf_propose_thesis`, `occupancy_audit`. Tests pin those files contain `put_research_artifact` and do not call `default_r2_put(` (stage file has no `default_r2_put` at all).
...
### OPEN (leftover `default_r2_put` callers under `packages/product/research/*.py`)

Live grep this turn — remaining files that still mention `default_r2_put`:

| File | Role this HEAD |
|------|----------------|
| `r2_io.py` | definition; dry_run staging inside `put_research_artifact` |
| `reconstitution_evidence.py` | `dry_run=True` only |
| `cf_mass_eval_run.py` | `put_local_fallback_artifacts` default `put_fn` still `default_r2_put` |
| `cf_daily_path_job.py` | remote `both_track.json` still **calls** `default_r2_put` (now raises; not Worker POST) |

Those leftover remote calls fail closed. They are **not** the overlay TOCTOU writer named at `5103b26b`. They are also **not** Worker `onlyIf`. Vestigial `python_r2_put_allowed()` still parses `QP_ALLOW_PYTHON_R2_PUT`; `default_r2_put` ignores it. Grep for `put_children_then_manifest_via_worker` outside `r2_io.py` and `tests/test_immutable_artifact.py` is still empty (callers go through `put_research_artifact`).

Do not treat P632B-05 as fully FIXED. Worker 409 stays FIXED. Overlay CLI TOCTOU is closed in tree. Leftover callers + live POST remain residuals.
...
- P632B-05 fully FIXED. Worker 409 is FIXED; overlay CLI TOCTOU is closed in tree; leftover `default_r2_put` callers remain fail-closed; live R2 unproven.
