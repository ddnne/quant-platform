# P632 independent review C revisit — catalog and pilot gates at `f224e7e`

**Lane:** independent C (not the implementer; isolation worktree `grok/p632-ind-C-revisit-f224e7e`; docs only)  
**HEAD:** `f224e7e` (`f224e7e922d93dfdcc14ae86578883cad337ebca`) (`contracts: missing SourceCapability V3 is fail-closed not COMPLETE`)  
**Prior freeze:** `07b4435` (`07b44355dc745b1a9b7f7c3c4eccbe123e7a171b`) — [`P632_ind_C_revisit.md`](P632_ind_C_revisit.md)  
**Earlier freeze:** `3ab87d0` — [`P632_ind_C_catalog_pilot.md`](P632_ind_C_catalog_pilot.md)  
...
**Catalog/pilot gates at `f224e7e` are unchanged vs `07b4435`.** Runtime blobs for the measured surfaces are identical (same git object). Live counts were re-measured at this HEAD, not copied.
...
HEAD **still matches** the previous PASS/HOLD scoreboard at `07b4435`. Counts, +N stops, reconstitution apply, Phase 7, Mass flags, and YAML overlay fail-closed are **unchanged**.

| Gate | At `07b4435` | At `f224e7e` | Live? |
|------|--------------|--------------|-------|
...
| `projection_age_seconds` | 181087 (~50.3 h) |
...
## What moved `07b4435` → `f224e7e` (this lane)

Runtime catalog/pilot blobs are **identical** to `07b4435` (git object SHA match):
...
|--------|---------------------|
| `44c7279` `test: catalog freeze is compiler digest plus one identity` | **Tests only.** Freeze n=2254 + digest pin live in `tests/test_catalog_compiler.py`. `tests/test_catalog_active_legacy.py` no longer re-compiles for n; still asserts `assert_catalog_ids_emit_frozen()["n_yaml"] == 0` and `go is False`. Does not flip Mass / READY / Phase 7. |
| `5af28dc` `docs: independent review C catalog/pilot revisit at 07b4435` | Prior C revisit text. Not a gate change. |
| `04af1ed` / `b23820e` / `6bee72a` (research coverage/PIT loaders) | Not catalog n, overlay, +N, reconstitution apply, Phase 7, or Mass. |
| `f224e7e` `contracts: missing SourceCapability V3 is fail-closed not COMPLETE` | SourceCapability completeness, not catalog/pilot. |
...
- **severity:** was P1 at `3ab87d0`; **status HOLD** at `07b4435` and **HOLD** at `f224e7e`
...
- **status:** OPEN (not live; yaml n=0; overlay env unset). Same as `07b4435`.
...
- **status:** OPEN (factorize) / HOLD (growth stopped; `start()` OFF). Same as `07b4435`.

---

## HOLD confirmations (re-measured at `f224e7e`; same as `07b4435`)

### YAML / AND `+N` growth — stopped

`packages/product/research/eval_flags.py:7-12` (blob unmoved vs `07b4435`):
