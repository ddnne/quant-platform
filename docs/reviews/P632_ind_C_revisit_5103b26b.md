# P632 independent review C revisit — catalog and pilot gates at `5103b26b`

**Lane:** independent C (not the implementer; isolation worktree `grok/p632-ind-C-revisit-5103b26b`; docs only)  
**HEAD:** `5103b26b` (`5103b26bb40b2d73b49d079e03f8cf5b9c2a4c58`) (`research: POST Worker children-then-manifest instead of raising`)  
**Prior freeze:** `ed94d504` (`ed94d5041969c3630b35c93927dcd1bb42f85c74`) — [`P632_ind_C_revisit_ed94d504.md`](P632_ind_C_revisit_ed94d504.md)  
**Earlier freezes:** `67fcbd7c` — [`P632_ind_C_revisit_67fcbd7c.md`](P632_ind_C_revisit_67fcbd7c.md); `40d1aa90` — [`P632_ind_C_revisit_40d1aa90.md`](P632_ind_C_revisit_40d1aa90.md); `f224e7e` — [`P632_ind_C_revisit_f224e7e.md`](P632_ind_C_revisit_f224e7e.md); `07b4435` — [`P632_ind_C_revisit.md`](P632_ind_C_revisit.md); `3ab87d0` — [`P632_ind_C_catalog_pilot.md`](P632_ind_C_catalog_pilot.md)  
...
**Catalog/pilot gates at `5103b26b` are unchanged vs `ed94d504`.** Runtime blobs for the measured surfaces are identical (same git object). Live counts were re-measured at this HEAD, not copied.
...
HEAD **still matches** the previous PASS/HOLD scoreboard at `ed94d504`. Counts, +N stops, reconstitution apply, Phase 7, Mass flags, and YAML overlay fail-closed are **unchanged**.

| Gate | At `ed94d504` | At `5103b26b` | Live? |
|------|---------------|---------------|-------|
...
| `projection_age_seconds` | 184818 (~51.3 h) |
...
[`original_plan_gap.md`](original_plan_gap.md) register — **unchanged** at this HEAD (banner now names `ed94d504`, this window’s pointer only; body freeze remains `e927b97`):
...
## What moved `ed94d504` → `5103b26b` (this lane)

Runtime catalog/pilot blobs are **identical** to `ed94d504` (git object SHA match):
...
`git rev-list --count ed94d504..5103b26b` = **17**. None of those commits change yaml n, compiled n, overlay, +N, reconstitution apply, Phase 7 `start()`, or Mass flags.

Window also touches `packages/product/research/evaluation_ir.py` and `packages/product/research/r2_io.py`. Those are Evaluation IR codec / children-then-manifest POST. Not catalog n, overlay, +N, reconstitution apply, Phase 7, or Mass.

| Commit | Effect on this lane |
|--------|---------------------|
| `7c567b9c` `docs: independent review C catalog/pilot revisit at ed94d504` | Prior C revisit text. Not a gate change. |
| `e68319d4` `docs: banner original-plan-gap register still holds at ed94d504` | Pointer only. Body freeze remains `e927b97`. Not a gate change. |
| `4661fb14` `worker: emit Evaluation IR encode/decode from schema.json` | IR codec. Not catalog n, overlay, +N, reconstitution apply, Phase 7, or Mass. |
| `5103b26b` `research: POST Worker children-then-manifest instead of raising` | `r2_io` POST order. Not catalog/pilot. |
| `34701984` / `fd660c40` / `e0699dbf` / `9da05fc5` (OTC year-index HTML / receipt `--index-text` / JQ weekend COMPLETE pin) | Ingest / Independent A. Not catalog/pilot. |
| `7dbcd9ea` print-only `ci-aggregate` first-deploy helper | Independent B / CI. Not catalog/pilot. |
| remaining docs / verify_ci inventory / Independent A–B revisit / review-index / MCP description commits | Not yaml n, compiled n, AND stop, reconstitution apply, Phase 7, Mass, or overlay. |
...
- **severity:** was P1 at `3ab87d0`; **status HOLD** at `ed94d504` and **HOLD** at `5103b26b`
...
- **status:** OPEN (not live; yaml n=0; overlay env unset). Same as `ed94d504`.
...
- **status:** OPEN (factorize) / HOLD (growth stopped; `start()` OFF). Same as `ed94d504`.

---

## HOLD confirmations (re-measured at `5103b26b`; same as `ed94d504`)

### YAML / AND `+N` growth — stopped

`packages/product/research/eval_flags.py:7-12` (blob unmoved vs `ed94d504`):
