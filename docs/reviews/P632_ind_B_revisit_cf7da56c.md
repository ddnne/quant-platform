# Independent review B revisit — at `cf7da56c`

**Reviewer:** independent Grok (isolation worktree; not the implementer)  
**Reviewed HEAD:** `cf7da56c` (`cf7da56c17260da2c2693540f28af91c849bd542`)  
**Branch at audit:** `grok/p632-ind-B-revisit-cf7da56c` (from `grok/phase63-ci-source-closure`)  
**Isolation worktree:** `/Users/taku/tmp/qp-p632-ind-B-revisit-cf7da56c`  
**Prior revisits:** `02fb6cbd` ([`P632_ind_B_revisit_02fb6cbd.md`](P632_ind_B_revisit_02fb6cbd.md)); `2b82ec7d` ([`P632_ind_B_revisit_2b82ec7d.md`](P632_ind_B_revisit_2b82ec7d.md)); `242c2484` ([`P632_ind_B_revisit_242c2484.md`](P632_ind_B_revisit_242c2484.md)); `3b64bdfc` ([`P632_ind_B_revisit_3b64bdfc.md`](P632_ind_B_revisit_3b64bdfc.md)); `5103b26b` ([`P632_ind_B_revisit_5103b26b.md`](P632_ind_B_revisit_5103b26b.md)); `ed94d504` ([`P632_ind_B_revisit_ed94d504.md`](P632_ind_B_revisit_ed94d504.md)); `67fcbd7c` ([`P632_ind_B_revisit_67fcbd7c.md`](P632_ind_B_revisit_67fcbd7c.md)); `40d1aa90` ([`P632_ind_B_revisit_40d1aa90.md`](P632_ind_B_revisit_40d1aa90.md)); `f224e7e` ([`P632_ind_B_revisit_f224e7e.md`](P632_ind_B_revisit_f224e7e.md)); `07b4435` ([`P632_ind_B_revisit.md`](P632_ind_B_revisit.md)); freeze `a48c600` ([`P632_ind_B_ci_authority.md`](P632_ind_B_ci_authority.md))  
**origin/main:** `b5c326a7f612563f2da4a84f08063a307ec38e0a` (feature HEAD is **not** an ancestor of `main`; not merged)  
**Scope:** re-measure Independent B — CI skip, `ci-aggregate` live producer, tokens, R2, budget — vs HEAD `cf7da56c`. Tree-level receipt auth and seven-worker `verify_ci` already landed before `07b4435`. This turn re-diffs those plus P632B-03..07 against `02fb6cbd`. Live GitHub check-runs, `main` protection, and `wrangler deployments list --name quant-platform-ci-aggregate` are this-turn measurements. P632B-01 code-lane delta: premium Worker unit tests now include **`dateMode` JSON contract** and **`natural_key_migration.test.ts`**; leftover Python `catalog.ts` grep is dropped; JSDA Worker unit tests pin fail-closed run-token / no invented COMPLETE. P632B-03 remains **HOLD** ([`P632B_03_gateway_token_service_binding_hold.md`](P632B_03_gateway_token_service_binding_hold.md) — HOLD, not closed). P632B-05 leftover `default_r2_put(` is still definition + dry_run (plus test pins).
...
| `projection_age_seconds` | 189581 (~52.66 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `02fb6cbd`. Empty diff on `platform/workers/ci-aggregate`. |
| Does `verify_ci` include `ci-aggregate` (7 workers)? | **Yes.** `WORKERS` has **7** paths; `tests/test_verify_ci_script.py` pins `len(WORKERS) == 7` and `for name in WORKERS: assert name in src`. Empty diff vs `02fb6cbd` on `scripts/verify_ci.sh` and the pin file. |
| Are GitHub check-runs still 0 live at `cf7da56c`? | **Yes.** HEAD `cf7da56c`, prior `02fb6cbd`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = []`. Actions workflows **0**. |
...
| Do premium / JSDA Worker tests now execute more identity/auth as unit tests? | **Yes.** At `02fb6cbd` premium tests already had catalog/availability/collection_receipts. This HEAD adds `catalog.test.ts` **dateMode**, new `natural_key_migration.test.ts`, drops leftover Python `catalog.ts` grep, and adds JSDA fail-closed run-token / no-COMPLETE units. That is a local `npm test` lane, **not** a merge-gate close. |
...
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | same | same (no `catalog_kind` in mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | **same** (empty grep for `catalog_kind` under mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | FIXED | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |

Independent P0 unresolved: **2**. Tree-level holes named at freeze for receipt auth and seventh-worker `verify_ci` stay closed. Overlay CLI put on `default_r2_put` is closed in tree. Python IR codec **and** types remain present and frozen in local `verify_ci`. Premium / JSDA Worker unit tests now execute more identity and inbound-auth cases instead of Python grep. The live merge gate still has **no honest producer**. Do not invent Coverage COMPLETE, Projection FRESH, B0 PASS, READY, or CI green.

Window vs `02fb6cbd`: `git rev-list --count 02fb6cbd..cf7da56c` = **14**. CI-surface deltas that are **not** a P0 close: `2218b861` (`catalog.test.ts` dateMode JSON contract); `8fc9fa30` (`natural_key_migration.test.ts` — Worker unit, not Python grep); `44642dfc` (drop leftover `catalog.ts` grep; Worker `catalog.test.ts` owns identity); `afd94e58` (JSDA fail-closed run-token / no invented COMPLETE in Worker unit). Remaining 10 commits are docs, including `a8f2eb95` (Independent B revisit at `02fb6cbd`). `platform/workers/ci-aggregate`, `scripts/ci_aggregate_first_deploy.sh`, `platform/workers/research-ai-gateway`, `platform/workers/research-mass-eval/src/ai_gateway_client.ts`, `platform/workers/research-mass-eval/src/http.ts`, `packages/product/research/r2_io.py`, `scripts/verify_ci.sh`, and `tests/test_verify_ci_script.py` are **empty diffs** against `02fb6cbd`. Print-only `scripts/ci_aggregate_first_deploy.sh` is **not** a producer.
...
**severity:** P0  
**affected:** `scripts/verify_ci.sh`; `scripts/verify_all.sh`; `tests/test_verify_ci_script.py`; `docs/ci/workers_builds.md`; `scripts/README.md`; `tests/README.md`; GitHub required context `ci-aggregate`; `platform/workers/ingestion-premium/src/{catalog,natural_key_migration,availability,collection_receipts,identity,index}.test.ts`; `platform/workers/ingestion-jsda/src/index.test.ts`  
**status:** **OPEN** (seven workers + IR schema + types `--check` honoring `scripts.types` + ALLOWED_FIELDS freeze + encode-keys freeze + TS codec-generated freeze + Python codec-generated freeze + Python types-generated freeze in `verify_ci` **FIXED**; premium dateMode / NK rebuild and JSDA fail-closed run-token Worker unit tests **added**; merge-gate producer still six `npm test` receipts)

### FIXED (must stay)

`scripts/verify_ci.sh` still lists seven Workers (`WORKERS` array empty-diff vs `02fb6cbd`):
...
Python Evaluation IR **types** file remains **present** and frozen (empty diff vs `02fb6cbd` on `verify_ci.sh`):
...
This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_02fb6cbd.md`](P632_verify_ci_02fb6cbd.md) is a later local PASS at `02fb6cbd`, not a GitHub status and not this SHA.

### Delta vs `02fb6cbd` (not a P0 close)

Premium / JSDA `npm test` now includes more identity and inbound-auth as **Worker unit tests**, not Python source-grep:

| File at `cf7da56c` | At `02fb6cbd` | Role |
|--------------------|---------------|------|
| `platform/workers/ingestion-premium/src/catalog.test.ts` | identity-only | **dateMode** / `dayParam` mapped from JSON contract (`2218b861`) |
| `platform/workers/ingestion-premium/src/natural_key_migration.test.ts` | **absent** | canonical NK rebuild / atomic swap / post-publish READY audit (`8fc9fa30`) |
| `tests/test_phase35_premium_set.py` | `catalog.ts` grep | leftover Python `catalog.ts` identity grep **dropped**; JSON contract vs Python ids remain (`44642dfc`) |
| `tests/test_identity_runtime_parity.py` | Worker source grep | `test_worker_rebuild_uses_canonical_fn_atomic_swap_and_post_publish_audit` **removed**; Worker unit owns it (`8fc9fa30`) |
| `platform/workers/ingestion-jsda/src/index.test.ts` | health/handlers only | missing/wrong/unbound `INGESTION_RUN_TOKEN` → 401; bodies do not leak token or invent COMPLETE (`afd94e58`) |

Those tests run inside `ingestion-premium` / `ingestion-jsda` `npm test`, which are **two of six lane receipts** *and* two of seven `verify_ci` workers. They do **not** make GitHub run `verify_ci`. They do **not** add Python / catalog freeze / IR / typecheck / dry-run to the merge-gate batch.
...
PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `cf7da56c`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “premium Worker now unit-tests dateMode / NK rebuild” or “JSDA Worker unit-tests run-token fail-closed” or “`ci-aggregate` is in `verify_ci.sh`” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `02fb6cbd` on `platform/workers/ci-aggregate`:
...
`scripts/ci_aggregate_first_deploy.sh` is an **empty diff** vs `02fb6cbd`. Default is dry-run / print-only. `--apply` without `CONFIRM_CI_AGGREGATE_CREATE=1` fails closed. With the confirm env, the script **still prints** `npx wrangler deploy` as a comment and does **not** exec it. Tests pin no live secret values. That helper is operator text, not a producer. This isolation turn did **not** run `--apply` and did **not** deploy.

### Live GitHub / Cloudflare (this isolation turn)

Fetched 2026-08-23T17:13Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
PR #1  head=cf7da56c  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=[]
```
...
1. has never been posted on `origin/main`, this reviewed SHA `cf7da56c`, prior revisit `02fb6cbd`, or earlier Independent B SHAs,
...
**status:** **OPEN / HOLD** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `02fb6cbd` on `ai_gateway_client.ts`. [`P632B_03_gateway_token_service_binding_hold.md`](P632B_03_gateway_token_service_binding_hold.md) is **HOLD, not closed**.
...
**status:** mixed. Production budget path empty vs `02fb6cbd`.
...
`89415105` still pins: created ledger has **zero** occupancy; `budget_id` presence is not a reserve. Comment: “Live Cloudflare Durable Object occupancy is unproven.” That is honesty, not a live occupancy proof and not a P1 close. Empty production-path diff vs `02fb6cbd`.
...
**severity:** P0 Worker overwrite **FIXED**; overlay CLI TOCTOU **FIXED in tree**; P1 leftover callers **OPEN** (unchanged vs `02fb6cbd` on production writers)  
...
**status:** mixed. Worker `onlyIf` empty vs `02fb6cbd`. `r2_io.py` empty vs `02fb6cbd`. Overlay CLI put remains closed. Live R2 POST unproven.

### FIXED (Worker 409)

`putJsonCreateOnly` (`http.ts:113-138`): `onlyIf: { etagDoesNotMatch: "*" }`; existing key with a different digest → 409. Child conflict still does not mint a manifest. Empty diff vs `02fb6cbd` on `http.ts`.
...
`default_r2_put` remote path **always** raises `WORKER_CHILDREN_THEN_MANIFEST_ERROR` (`r2_io.py:361`). Overlay `QP_ALLOW_PYTHON_R2_PUT=1` does **not** resurrect `wrangler r2 object put`. Empty diff vs `02fb6cbd` on `r2_io.py`.
...
Job-artifact callers this HEAD that use `put_research_artifact`: `cf_mass_eval_job`, `cf_mass_eval_stage`, `cf_mass_eval_run`, `cf_daily_path_job`, `cf_cost_verify`, `cf_propose_thesis`, `occupancy_audit`. Unchanged vs `02fb6cbd`.
...
Product/research remaining callers are unchanged vs `02fb6cbd` (definition + dry_run). Mentions of the name in `r2_io.py` comments / `__all__` remain.
...
- Premium dateMode / NK rebuild tests, dropped `catalog.ts` grep, or JSDA fail-closed run-token tests close P632B-01. They are local `npm test` coverage, not the merge gate.
