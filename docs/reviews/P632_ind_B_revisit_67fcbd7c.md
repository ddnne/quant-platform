# Independent review B revisit — at `67fcbd7c`

**Reviewer:** independent Grok (isolation worktree; not the implementer)  
**Reviewed HEAD:** `67fcbd7c` (`67fcbd7cd56847a9fc0fba7bcefbd743b43fc106`)  
**Branch at audit:** `grok/p632-ind-B-revisit-67fcbd7c` (from `grok/phase63-ci-source-closure`)  
**Prior revisits:** `40d1aa90` ([`P632_ind_B_revisit_40d1aa90.md`](P632_ind_B_revisit_40d1aa90.md)); `f224e7e` ([`P632_ind_B_revisit_f224e7e.md`](P632_ind_B_revisit_f224e7e.md)); `07b4435` ([`P632_ind_B_revisit.md`](P632_ind_B_revisit.md)); freeze `a48c600` ([`P632_ind_B_ci_authority.md`](P632_ind_B_ci_authority.md))  
**origin/main:** `b5c326a7f612563f2da4a84f08063a307ec38e0a` (feature HEAD is **not** an ancestor of `main`; not merged)  
**Scope:** re-measure Independent B — CI skip, `ci-aggregate` live producer, tokens, R2, budget — vs HEAD `67fcbd7c`. Tree-level receipt auth and seven-worker `verify_ci` already landed before `07b4435`. This turn re-diffs those plus P632B-03..07 against `40d1aa90`.
...
| `projection_age_seconds` | 183093 (~50.86 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `40d1aa90` (`ff053a3`). |
| Does `verify_ci` include `ci-aggregate` (7 workers)? | **Yes.** `WORKERS` has **7** paths; `tests/test_verify_ci_script.py` pins `len(WORKERS) == 7` and `for name in WORKERS: assert name in src`. Empty diff vs `40d1aa90` on `scripts/verify_ci.sh` and the pin test. |
| Are GitHub check-runs still 0 live at `67fcbd7c`? | **Yes.** HEAD `67fcbd7c`, prior `40d1aa90`, `f224e7e`, `07b4435`, freeze `a48c600`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = null`. Actions workflows **0**. |
...
## Scoreboard vs `40d1aa90` / `f224e7e` / `07b4435` / `a48c600`

| ID | Topic | Sev | At `a48c600` | At `07b4435` | At `f224e7e` | At `40d1aa90` | At `67fcbd7c` |
|----|-------|-----|--------------|--------------|--------------|---------------|---------------|
| P632B-01 | `verify_all` skip vs `verify_ci` authority | P0 | OPEN (scripts split FIXED; `ci-aggregate` absent from `verify_ci`; merge gate not `verify_ci`) | OPEN (`ci-aggregate` in `verify_ci` FIXED; GitHub still does not run `verify_ci`) | OPEN (IR jsonschema + `wrangler types --check` in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`npm run types -- --check` + ALLOWED_FIELDS freeze in `verify_ci` FIXED; merge gate still six lane receipts) | **OPEN** (seven-worker `verify_ci` unchanged vs `40d1aa90`; merge gate still six lane receipts) |
...
| P632B-03 | Shared `GATEWAY_TOKEN` / `MASS_EVAL_TOKEN` | P1 | OPEN (header mix-up FIXED) | not re-diffed | OPEN (mass-eval still sends `GATEWAY_TOKEN`) | OPEN (mass-eval still sends `GATEWAY_TOKEN`) | **OPEN** (empty diff vs `40d1aa90`: mass-eval still sends `GATEWAY_TOKEN`) |
| P632B-04 | Edge budget double-spend | P0/P1 | FIXED same-DO occupancy; OPEN P1 residuals | not re-diffed | same | same | **same** (P0 occupancy still FIXED; P1 residuals still OPEN; `89415105` honesty test only — live DO occupancy unproven) |
...
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | same | **same** (no `catalog_kind` in mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | FIXED | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |
...
Window vs `40d1aa90`: `git rev-list --count 40d1aa90..67fcbd7c` = **17**. CI-surface deltas that are **not** a P0 close: `e854ae56` (operator steps in `docs/ci/workers_builds.md` name live Worker **absent**); `61c14a0d` (`put_children_then_manifest_via_worker` fail-closes; no HTTP client, no CLI put fallback); `89415105` (in-memory test: `budget_id` create is not a reserve; comment pins live DO occupancy unproven). `platform/workers/ci-aggregate`, `scripts/verify_ci.sh`, and `tests/test_verify_ci_script.py` are **empty diffs** against `40d1aa90`. Tokens / catalog / GHA surfaces are **empty diffs** against `40d1aa90`.
...
**status:** **OPEN** (seven workers + IR schema + types `--check` honoring `scripts.types` + ALLOWED_FIELDS freeze in `verify_ci` **FIXED**; merge-gate producer still six `npm test` receipts)

### FIXED (must stay)

`scripts/verify_ci.sh` still lists seven Workers (empty diff vs `40d1aa90`):
...
`tests/README.md` names `scripts/verify_ci.sh` as mandatory local CI; `verify_all.sh` is the helper. `scripts/README.md` leads with `verify_ci.sh` (seven workers including `ci-aggregate`). `docs/ci/workers_builds.md:94-100` still says six-lane `npm test` receipts “skip Python/catalog and are **not** `verify_ci`”. Merge “requires GitHub context `ci-aggregate` after authenticated receipts **and** `verify_ci`”.

This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_40d1aa90.md`](P632_verify_ci_40d1aa90.md) is a later local PASS at `40d1aa90`, not a GitHub status and not this SHA.
...
| `verify_ci.sh` | pytest + catalog + IR jsonschema + ALLOWED_FIELDS freeze + **7** workers ci/test/typecheck/dry-run/types `--check` | no |
...
`scripts/README.md:17` still says Evaluation IR golden/schema **presence** in the mandatory-CI sentence; `verify_ci.sh` jsonschema-validates. Honesty residual, not a merge-gate close.

PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `67fcbd7c`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “`ci-aggregate` is in `verify_ci.sh`” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `40d1aa90` on `platform/workers/ci-aggregate`:
...
`e854ae56` updated operator steps: “live: Worker absent from account as of Independent B f224e7e/40d1aa90 — HUMAN create.” This turn re-measures: still absent.

### Live GitHub / Cloudflare (this isolation turn)

Fetched 2026-08-23T15:22Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
PR #1  head=67fcbd7c  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=null
```
...
1. has never been posted on `origin/main`, this reviewed SHA `67fcbd7c`, prior revisit `40d1aa90`, prior revisit `f224e7e`, prior revisit `07b4435`, or freeze `a48c600`,
...
**status:** **OPEN** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `40d1aa90`.
...
**status:** mixed. Production budget path empty vs `40d1aa90`. `89415105` adds an in-memory test only.
...
`89415105` pins: created ledger has **zero** occupancy; `budget_id` presence is not a reserve. Comment: “Live Cloudflare Durable Object occupancy is unproven.” That is honesty, not a live occupancy proof and not a P1 close.
...
**status:** mixed. Worker HTTP empty diff vs `40d1aa90`. Python path **stricter** (`61c14a0d`) but not a fence.
...
Delta vs `40d1aa90`: `put_children_then_manifest_via_worker` is the Worker-client entry. Unbound Worker URL/token fail closed. There is **no** CLI put fallback. This commit does **not** ship an HTTP client; remote always raises `WORKER_CHILDREN_THEN_MANIFEST_ERROR` until Worker POST is wired. `QP_ALLOW_PYTHON_R2_PUT=1` does not grant CLI put on this path. That is fail-closed by default on the new entry. It is **not** Worker `onlyIf`. Existing `default_r2_put` callers remain. Do not treat this as P632B-05 FIXED.
...
- `scripts/README.md:17` still says Evaluation IR golden/schema **presence**; `verify_ci.sh` jsonschema-validates and freezes generated ALLOWED_FIELDS. Honesty residual. Merge gate is still not `verify_ci`.
