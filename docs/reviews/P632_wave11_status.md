...
**PR:** https://github.com/ddnne/quant-platform/pull/1 — `MERGEABLE` / mergeState **`BLOCKED`** (required `ci-aggregate` has not posted). Head OID this turn: `02fb6cbd`.
...
| Wave-11 (this file) | `02fb6cbd` | this re-diff |
...
```text
commits/02fb6cbd/check-runs  total_count: 0
commits/02fb6cbd/status      state: pending, total_count: 0
PR #1                       mergeable: MERGEABLE
                            mergeStateStatus: BLOCKED
                            statusCheckRollup: []
                            headRefOid: 02fb6cbd70f2039cd47bcf7a15838182842f3426
...
Cron PASS, raw-retention COMPLETE, and row-count growth are **not** Coverage COMPLETE or READY. Local `verify_ci` at `2b82ec7d` is **not** a posted GitHub context. Print-only first-deploy is **not** a producer. Same generation as wave-10 (`projgen-ef18b4f86ee946048161d25e2a30a2a8`). Age grew 187980 → 188717. Floors and 4-PARTIAL set are unchanged.
...
Wave-11 **tree** honesty (five premium Worker unit replacements of Python greps: catalog identity, availability policy, identity JST clocks, raw-page retain, coverage-segment plan) is not a GO. Brief §8 `Pilot_GO` is a conjunction; live legs still fail: Projection **STALE**, `refresh_success=false`, B0 **UNKNOWN**, READY **null**, `applied_feed_cursor=null`, required GitHub `ci-aggregate` **not posted** (`check-runs total_count: 0`), Coverage still V2 **22 / 4 PARTIAL**, independent P0 unresolved ≠ 0 (live merge gate; **ci-aggregate Worker absent**).
...
|-----|---------|---------------|-------------------|
| `23a5cbb9` | premium catalog identity is Worker unit not Python grep | `ingestion-premium/src/catalog.test.ts` (`it(` **1**) owns JSON `dataset_id` set-equality vs `PREMIUM_CORE_DATASETS` (23). Python `test_phase35_availability.py` no longer greps `catalog.ts` for that identity. | `tests/test_phase35_premium_set.py` `test_typescript_catalog_matches_python` still greps `catalog.ts` (`contractDocument.datasets` / `rawContracts.map` / no hardcoded `id: "equities_master"`). Not merge-gate. |
| `8fc13e24` | premium availability policy is Worker unit not Python grep | `availability.test.ts` (`it(` **8**) owns unknown-dataset `ingest_time_conservative`, wrapper fail-safe, missing-field fallback. Python file keeps **execution** of `pick_available_at` / `session_close_jst`, not Worker-body greps. | Not Dataset COMPLETE. Not a secret bind. Not merge-gate. |
| `5ac9cce1` | premium identity JST clocks are Worker unit not Python grep | `identity.test.ts` `it(` **1 → 2**: `sessionCloseJst` owns `15:00` / `15:30` / morning `11:30`. Python dropped the Worker-body clock greps (`2024-11-05`, `15:30:00`, `15:00:00`, `11:30:00`). | `tests/test_identity_runtime_parity.py` stays real Py↔TS **execution** parity. Not merge-gate. |
| `0383311f` | premium raw-page retain is Worker unit not Python grep | `index.test.ts` grew retain/ACQUIRED/FAILED pages (`it(` **5 → 11** with the coverage-segment landing). Python `test_phase6_data_access.py` no longer greps Worker raw-page strings. Raw write is **ACQUIRED**, never Coverage COMPLETE. | Live `ops_status.raw_retention` still emits `complete` only. Live Worker is not this SHA. Not Dataset COMPLETE. |
| `0fee1b1e` | premium coverage-segment plan is Worker unit not Python grep | `collection_receipts.test.ts` (`it(` **2**) plus façade `index.test.ts` plan pins: query-unit months, `event_driven` null items, non-canonical window does not write required segments. Python `test_phase61_coverage_v2.py` dropped Worker-body greps (remaining `collection_receipts` hits are D1 table / SQL migration, not Worker source). | Live MCP still V2 STALE **22 / 4 PARTIAL**. Planning UNKNOWN/null is not Coverage COMPLETE. |

Independent revisits / inventories at `2b82ec7d` (`cedc353e` A, `5e4abd42` B, `ea2a3e23` C, `b37975bc` wave-10, `156da9cb` P, `93e0b942` `verify_ci`, `dad76c17` original-plan-gap, `40514f42` residual SoT, `4b7aeb18` §10 remaining mixed, `02fb6cbd` review index) are freezes / operator notes vs the wave-10 SHA, not live GO. They do **not** yet name the five premium Worker unit replacements. Independent A/B/C freeze SHAs are `2b82ec7d`. Catalog identity (`specs/research_catalog/`, `catalog_ids.ts`, `daily_path.ts`) is an empty diff `2b82ec7d..02fb6cbd`. Premium **production** `*.ts` (not tests) is also an empty diff.

This isolation did **not** re-run `scripts/verify_ci.sh` at `02fb6cbd`. The `2b82ec7d` local PASS ([`P632_verify_ci_2b82ec7d.md`](P632_verify_ci_2b82ec7d.md): **1505 passed / 4 skipped**; `ingestion-premium` was **14** tests at that SHA) is the wave-10 reviewed SHA, not this HEAD, and is **not** merge-gate. Prior `242c2484` PASS (**1502 passed / 4 skipped**) is still not this SHA.

---

## The 15 commits after `2b82ec7d`

| SHA | Landing | Lane |
|-----|---------|------|
| `23a5cbb9` | premium catalog identity is Worker unit not Python grep | L / P |
| `8fc13e24` | premium availability policy is Worker unit not Python grep | L / P |
| `5ac9cce1` | premium identity JST clocks are Worker unit not Python grep | L / P |
| `0383311f` | premium raw-page retain is Worker unit not Python grep | L / P |
| `0fee1b1e` | premium coverage-segment plan is Worker unit not Python grep | L / P |
| `cedc353e` | independent review A revisit at `2b82ec7d` | docs (A freeze) |
| `5e4abd42` | independent review B revisit at `2b82ec7d` | docs (B freeze) |
| `ea2a3e23` | independent review C catalog/pilot revisit at `2b82ec7d` | docs (C freeze) |
| `b37975bc` | wave-10 status after commits vs `242c2484` | docs (wave-10 freeze) |
| `156da9cb` | 6.3.2 P test inventory at `2b82ec7d` | docs (P) |
| `93e0b942` | P632 `verify_ci` code-lane result at `2b82ec7d` | docs (C; not merge-gate) |
| `dad76c17` | banner original-plan-gap register still holds at `2b82ec7d` | docs |
| `40514f42` | residual SoT banner HEAD `2b82ec7d` vs `origin/main` `b5c326a` | docs |
| `4b7aeb18` | §10 remaining mixed at `2b82ec7d` | docs (Q freeze) |
| `02fb6cbd` | review index names HEAD `2b82ec7d` vs `origin/main` `b5c326a` | docs |

Docs commits in this window (`cedc353e`, `5e4abd42`, `ea2a3e23`, `b37975bc`, `156da9cb`, `93e0b942`, `dad76c17`, `40514f42`, `4b7aeb18`, `02fb6cbd`) are freezes / operator notes, not live GO. Independent A/B/C revisits are authored vs `2b82ec7d` and do **not** yet name catalog / availability / identity-JST / raw-page / coverage-segment Worker units. `4b7aeb18` remaining-extracts freeze is authored vs `2b82ec7d` (TypedDict / shared reader already **DONE** in that freeze); this re-diff is vs `02fb6cbd`.

---

## A–S vs `02fb6cbd` (after the 15)

Prior registers: [`P632_brief_leaks.md`](P632_brief_leaks.md) vs `3ab87d0`; [`P632_wave2_status.md`](P632_wave2_status.md) vs `07b4435`; [`P632_wave3_status.md`](P632_wave3_status.md) vs `f224e7e`; [`P632_wave4_status.md`](P632_wave4_status.md) vs `40d1aa90`; [`P632_wave5_status.md`](P632_wave5_status.md) vs `67fcbd7c`; [`P632_wave6_status.md`](P632_wave6_status.md) vs `ed94d504`; [`P632_wave7_status.md`](P632_wave7_status.md) vs `5103b26b`; [`P632_wave8_status.md`](P632_wave8_status.md) vs `3b64bdfc`; [`P632_wave9_status.md`](P632_wave9_status.md) vs `242c2484`; [`P632_wave10_status.md`](P632_wave10_status.md) vs `2b82ec7d`. This table is the wave-11 re-diff, not a rewrite of those files.
...
| Sub-item | At `2b82ec7d` (wave-10) | After wave-11 |
|----------|-------------------------|---------------|
...
| `verify_ci` covers gate Worker | **FIXED** + python types freeze. Local PASS documented at `242c2484`, not `2b82ec7d`. | **FIXED** (empty diff on `scripts/verify_ci.sh`). Local PASS now documented at `2b82ec7d` ([`P632_verify_ci_2b82ec7d.md`](P632_verify_ci_2b82ec7d.md)), not this HEAD, not GitHub. |
| Branch protection on `main` | **FIXED** (requires `ci-aggregate`) | **FIXED** (setting). `app_id: null`. Not a passing receipt. |
| GitHub check-runs / statuses at HEAD | **OPEN** (`2b82ec7d` `total_count: 0`) | **OPEN** — this turn `02fb6cbd` `total_count: 0`; `/status` `pending` / `0`. PR #1 `MERGEABLE` / `BLOCKED`; `statusCheckRollup: []`. |
...
Empty diff vs `2b82ec7d` on Worker production sources for lockfiles / jitter / fetch extract. Premium Worker **unit** tests grew in place (`23a5cbb9` / `8fc13e24` / `5ac9cce1` / `0383311f` / `0fee1b1e`; first-party files **22 → 25**; premium `it(` **14 → 32**). That is test-surface honesty, not a clean-checkout matrix.

Local `verify_ci` PASS is now **documented** at `2b82ec7d` ([`P632_verify_ci_2b82ec7d.md`](P632_verify_ci_2b82ec7d.md): 7 workers including `ci-aggregate` 13 tests; **1505 passed / 4 skipped**; `ingestion-premium` was **14** tests at that SHA). That is a different SHA than this HEAD. Clean-checkout matrix **not** executed at `02fb6cbd` in this isolation.

Do not invent PASS at `02fb6cbd`.

### C. Authoritative CI script

**status: PARTIAL** (`verify_ci.sh` fail-closed, 7 workers, IR schema, generated `ALLOWED_FIELDS`, encode-key lock, generated TS codec freeze, freeze-**call** pin, generated Python codec freeze, generated Python types freeze, types flags, **local PASS at `2b82ec7d`**, print-only first-deploy helper named) / **OPEN** (merge gate is live GitHub context; producer Worker **absent**)

- Authoritative: `scripts/verify_ci.sh`. `WORKERS` still includes `platform/workers/ci-aggregate`. Missing lockfile/dir/script fails. No skip of missing `node_modules`. No `--legacy-peer-deps`. Empty diff vs `2b82ec7d`.
...
- Local PASS at `2b82ec7d`: 1505 passed / 4 skipped; wall 176.22s; `verify_ci: ok`. **Not** merge-gate. **Not** this HEAD. This isolation did not re-run the script after the wave-11 code landings (`23a5cbb9`, `8fc13e24`, `5ac9cce1`, `0383311f`, `0fee1b1e`).
...
Planner **does** clip through SourceCapability SoT. That is a wire, not Dataset COMPLETE 23. Live MCP is STALE and still advertises V2 floors. Empty diff vs wave-10 on the four files.
...
Empty production diff vs `2b82ec7d` on planner / sealer / shared reader. Wave-11 did not land another OTC wire.
...
Unchanged vs wave-10 live. `latest_ready_snapshot`: **null**. Digest-bound predicate ≠ a bound READY generation. STALE V2 PARTIAL keeps READY honest-false. That is intended; it is not a GO.
...
Unchanged vs wave-10. Caps unchanged (`auto_promotion: false`). Live occupancy / double-spend under production traffic: **not** measured this wave. String `budget_id` is still not the reserve. Empty diff vs `2b82ec7d` on `research-ai-gateway`.

### L. Worker public boundary

**status: PARTIAL** (preview vs production split; secrets proxy **Worker unit**; **premium catalog / availability / identity-JST / raw-page / coverage-segment Worker unit**) / **OPEN** (shared bearer still required on remaining surfaces; leftover Python `catalog.ts` grep) / **HOLD** (`workers_dev` kept where documented; GATEWAY service-binding residual) / **HUMAN** (secret bind)

Unchanged vs wave-10 production `workers_dev=false` on research-ai-gateway, research-mass-eval, ingestion-jsda, ingestion-premium. **Kept true:** quant-ops-mcp (OAuth callback), ingestion-secrets (token-gated local), ci-aggregate (receipt POST host). Treat **kept** as **HOLD**. Dual `GATEWAY_TOKEN` / service-binding residual: **OPEN** / **HOLD** (`7221c588`; mass-eval `ai_gateway_client.ts` still sends `GATEWAY_TOKEN` as `X-Gateway-Token`; empty diff vs `2b82ec7d`). Ops MCP must not grow SQL / fetch / ingest / delete / READY publish. Not re-opened.

Wave-11 vs wave-10: five premium Worker unit replacements (`23a5cbb9` / `8fc13e24` / `5ac9cce1` / `0383311f` / `0fee1b1e`). That is contract-vs-unit split, **not** a secret bind. `tests/test_phase35_premium_set.py` still greps `catalog.ts` for import/`rawContracts.map`/no hardcoded `id` — leftover echo, not a bind.
...
Empty diff vs `2b82ec7d` on `r2_io.py` (431) and `reconstitution_evidence.py`. Remaining `default_r2_put` production caller: `reconstitution_evidence` dry-run-only (`default_r2_put(..., dry_run=True)`). `put_research_artifact` still stages locally via `default_r2_put` when `dry_run=True`. Overlay identity **HOLD**.
...
Independent C at `2b82ec7d` ([`P632_ind_C_revisit_2b82ec7d.md`](P632_ind_C_revisit_2b82ec7d.md)): YAML n = **0**; compiled freeze n = **2254**; `yaml_overlay_allowed()` **False**; `go: false`. Compact `family + template + parameter matrix` **not** implemented. Catalog identity is an empty diff `2b82ec7d..02fb6cbd` (`specs/research_catalog/`, `catalog_ids.ts`, `daily_path.ts`).
...
Empty production diff vs `2b82ec7d` on IR sources. Codec bodies unchanged: generated Python codec; generated TS codec (239); Worker façade `evaluation_ir.ts` (39); emit helper `evaluation_ir_emit.py` (823); façade `evaluation_ir.py` (646); TypedDict `evaluation_ir_types.generated.py` (47). Encode object keys remain schema properties; grade remains `jobCandidateGrade` / `job_candidate_grade`. Unknown fields still fail; version stays `evaluation-ir/v1`.
...
**status: OPEN** (inventory not closed) / **PARTIAL** (Lane 17 audit + collect freezes + digest identity + READY fixture honesty + event-zero tip/index pins + secrets Worker unit + **premium catalog / availability / identity-JST / raw-page / coverage-segment Worker unit**)

See [`P632_test_inventory.md`](P632_test_inventory.md) (`3ab87d0`, collected **1353**), [`P632_test_inventory_now.md`](P632_test_inventory_now.md) (`07b4435`, **1379**), [`P632_test_inventory_40d1aa90.md`](P632_test_inventory_40d1aa90.md) (**1426**), [`P632_test_inventory_67fcbd7c.md`](P632_test_inventory_67fcbd7c.md) (**1448**), [`P632_test_inventory_ed94d504.md`](P632_test_inventory_ed94d504.md) (**1470**), [`P632_test_inventory_5103b26b.md`](P632_test_inventory_5103b26b.md) (**1496**), [`P632_test_inventory_3b64bdfc.md`](P632_test_inventory_3b64bdfc.md) (**1503**), [`P632_test_inventory_242c2484.md`](P632_test_inventory_242c2484.md) (**1506**), [`P632_test_inventory_2b82ec7d.md`](P632_test_inventory_2b82ec7d.md) (**1509** collected; `tests/test_*.py` **154**; Worker first-party **22**; YAML **0**). Local `verify_ci` at `2b82ec7d` reported **1505 passed, 4 skipped**.

This HEAD `git ls-files tests/test_*.py` = **154** (no Python module add/delete). Worker first-party test **files** **22 → 25** (21 `*.test.ts` + 4 `*.test.mjs`): added `availability.test.ts`, `catalog.test.ts`, `collection_receipts.test.ts`. Premium `it(` **14 → 32**. Wave-10 inventory named Phase 35 availability / identity / catalog Python greps **OPEN**; wave-11 landed Worker units for those three plus raw-page retain and coverage-segment plan. Remaining Worker-body grep: `tests/test_phase35_premium_set.py` `test_typescript_catalog_matches_python` still reads `catalog.ts`. JSON contract vs Python `PREMIUM_CORE_DATASETS` set-equality is not a Worker-body grep; keep that identity. `tests/test_identity_runtime_parity.py` is execution parity, not echo. This wave did **not** re-run `pytest --collect-only`. Do not invent `tests_after`. Count growth of Worker units is a mechanism replacement, not a consolidation win, and not a GO.

### Q. Whole-repo refactor (authority split)

**status: PARTIAL** / **OPEN** / **HOLD** (named live-math files)

§10 remaining-extracts freeze is authored vs `2b82ec7d` ([`docs/phase63_refactor_plan.md`](../phase63_refactor_plan.md); `4b7aeb18`). That freeze already records TypedDict generation **DONE** and shared official-index reader **DONE**. Wave-11 landings vs that freeze are **test-surface**, not a new mixed-authority extract:

| §10.3 later | At `2b82ec7d` (wave-10) | After wave-11 |
|-------------|-------------------------|---------------|
| 1 BackfillPlanner vs `plan_required_segments` | **DONE** (JQ jobs from required segments; pipeline held OTC HTML; `issue_*` `--index-text`; BackfillPlanner `index_text`; shared local HTML reader; `cf_premium_backfill --index-text`). | **DONE** — empty production diff. Live MCP still V2 STALE. |
...
| 5 `verify_all` vs `verify_ci` | **HOLD** (local PASS at `242c2484` is not merge-gate; python types freeze; print-only first-deploy still does not create the producer) | **HOLD** (local PASS at `2b82ec7d` is not merge-gate; print-only first-deploy still does not create the producer) |
...
`RECONSTITUTION_APPLY = False`. Agent must not flip apply. Reconstitution evidence stays dry-run-only (`default_r2_put` local stage). Empty diff vs `2b82ec7d` on `reconstitution_evidence.py`.

### S. Controlled Pilot

**status: FIXED** (OFF) — GO conditions **OPEN** / unmet

Worker: `MASS_RESEARCH = "NO-GO"`, `PHASE7 = "OFF"`, `READY_DECLARED = "false"` (`research-mass-eval/wrangler.toml`). Python deny-by-default, `go: False`. **Do not** run the one-shot paper loop. Mass remains **NO-GO**. Auto promotion **OFF**. Live broker **OFF**. Independent C at `2b82ec7d`: catalog/pilot P0 unresolved **0** (no live arming); that is not Phase 7 GO. Catalog identity empty vs this HEAD.

---

## Independent P0 scoreboard (tree vs live)

| ID | At `2b82ec7d` (wave-10) | After wave-11 (`02fb6cbd`) |
|----|-------------------------|----------------------------|
| IND-A-DOMAIN | **FIXED**. Live STALE still V2 `2006-08-13`. | **FIXED** (not reopened). Live still STALE. Independent A freeze now at `2b82ec7d`. |
| IND-A-JSDA-PHANTOM | **FIXED** (tree) + shared local HTML reader + `cf_premium_backfill --index-text`. Live still **8784 / 5886**. PARSE_ZERO stays gap. | **FIXED** (not reopened; empty production diff). Live still **8784 / 5886**. PARSE_ZERO stays gap. Independent A freeze file is still vs `2b82ec7d`. |
| IND-A-PIT-BYPASS | **FIXED** | **FIXED** (not reopened) |
| IND-A-FORGED-RECEIPT | **FIXED** | **FIXED** (not reopened) |
| IND-A-FALSE-COMPLETE | **PARTIAL** (shared reader / backfill omit still `None`, not weekend COMPLETE). Live MCP still `complete`. Independent A tree P0 = 0. | **PARTIAL** (premium raw-page retain unit still writes **ACQUIRED**, never Coverage COMPLETE). Live MCP still `complete`. Independent A freeze at `2b82ec7d` still tree P0 = 0. |
| IND-A-READY-DEPS | **FIXED**. Live READY **null**. | **FIXED**. Live **null**. |
| P632B-01 `verify_all` vs `verify_ci` | **PARTIAL** — local PASS at `242c2484`; python types freeze; live merge gate still not `verify_ci`; producer Worker **absent** | **PARTIAL** — local PASS at `2b82ec7d` documented; live merge gate still not `verify_ci`; producer Worker **absent** |
...
| P632B-03 `GATEWAY_TOKEN` / `MASS_EVAL_TOKEN` | **OPEN** / **HOLD** (`7221c588` pointer; empty diff on `ai_gateway_client.ts`). Unbound mass-eval token on children-then-manifest stays **503**. | **OPEN** / **HOLD** (empty diff vs `2b82ec7d` on `ai_gateway_client.ts`). Unbound mass-eval token stays **503**. |
| P632B-05 Python R2 TOCTOU | **PARTIAL** — daily-path + mass-eval-run Worker put. Remaining `default_r2_put` caller: reconstitution dry_run only. Live POST unproven. Overlay helper **HOLD**. | **PARTIAL** — empty diff vs `2b82ec7d`. Remaining `default_r2_put` caller: reconstitution dry_run only. Live POST unproven. Overlay helper **HOLD**. |
| C-YAML load overlay | **FIXED**. +N **HOLD**. Independent C freeze at `242c2484` (P0 unresolved **0**; not Phase 7 GO). Catalog identity empty vs `2b82ec7d`. | **FIXED**. +N **HOLD**. Independent C freeze now at `2b82ec7d` (P0 unresolved **0**; not Phase 7 GO). Catalog identity empty vs this HEAD. |

Independent P0 unresolved ≠ 0 (live CI never posted; **ci-aggregate Worker absent**). Independent A tree P0 = 0. Independent B live P0 = **2**. Independent C catalog/pilot P0 = 0. `Pilot_GO.IndependentReview_P0_Zero` stays **OPEN**. Tree premium Worker units are not a live Coverage COMPLETE.
...
| Six_Workers_Clean | lockfiles **FIXED**; `verify_ci` 7 workers **FIXED** (script); retry jitter `getRandomValues` **FIXED** (tree); local PASS at `2b82ec7d` **FIXED** (docs); python types freeze **FIXED** (tree); clean-checkout matrix at this HEAD **OPEN** |
...
- A rewrite of [`P632_brief_leaks.md`](P632_brief_leaks.md), [`P632_wave2_status.md`](P632_wave2_status.md), [`P632_wave3_status.md`](P632_wave3_status.md), [`P632_wave4_status.md`](P632_wave4_status.md), [`P632_wave5_status.md`](P632_wave5_status.md), [`P632_wave6_status.md`](P632_wave6_status.md), [`P632_wave7_status.md`](P632_wave7_status.md), [`P632_wave8_status.md`](P632_wave8_status.md), [`P632_wave9_status.md`](P632_wave9_status.md), or [`P632_wave10_status.md`](P632_wave10_status.md).
- A live Coverage remeasure. Premium Worker units are git; MCP is **STALE**.
- Dataset COMPLETE 23, OTC COMPLETE, B0 PASS, READY, Phase 6.3.2 COMPLETE, Phase 6.4 COMPLETE, or Phase 7 GO.
- Proof that `verify_ci.sh` is green at `02fb6cbd`. Local PASS at `2b82ec7d` is not this SHA and is not merge-gate.
...
Wave-11 **tree** honesty landed. Phase 6.3.2 remains **NOT COMPLETE**. Phase 6.4 remains **NOT COMPLETE**. Controlled Pilot remains **NO-GO**.
