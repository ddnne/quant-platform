# Phase 6.3.2 P — test inventory at `3b64bdfc`

**Kind:** read-only inventory. Does **not** delete tests. Does **not** invent
`tests_after`. Does **not** flip GO.  
**SHA counted:** `3b64bdfc9a41be76a6e4e881aaea1ff9751443ed` (`3b64bdfc`)  
`docs: 6.3.2 wave-7 status after 17 commits vs 5103b26b`  
**Isolation worktree:** `/private/tmp/qp-p632-test-inventory-3b64bdfc` on
`docs/p632-test-inventory-3b64bdfc` off `origin/grok/phase63-ci-source-closure`.  
...
| Surface | How counted (this isolation tree at `3b64bdfc`) |
|---------|-------------------------------------------------|
| Python collected | `/Users/taku/GitHub/quant-platform/.venv/bin/python -m pytest --collect-only -q tests/` from this worktree. Python 3.11.15, pytest 9.1.1 (`pyproject.toml` `addopts = "-q"`). **PASS** (exit 0). Quiet `-q` prints 153 per-file counts summing to **1503**. Confirming with addopts cleared: `1503 tests collected in 1.44s`. |
...
| Metric | Brief §P (`tests_before`) | `5103b26b` inventory | This SHA (`3b64bdfc`) |
|--------|--------------------------:|---------------------:|----------------------:|
| **tests_before (collected)** | ~**1282** | — | — |
| **tests_after (collected)** | *not invented* | *not invented* | *not invented* |
| **collected now (actual)** | — | **1496** | **1503** |
| `tests/test_*.py` files | — | **153** | **153** |
| Worker `*.test.ts` | — | **16** | **16** |
| Worker `*.test.mjs` | — | **4** | **4** |
| Worker first-party test files | — | **20** | **20** |
| Worker source `it(` / `test(` | — | **145** | **146** (not vitest collect) |
| `specs/research_logics` YAML | — | **0** | **0** (dir exists; only `README.md`) |

`1503 > 1496 > 1470 > 1448 > 1426 > 1379 > 1282`. Later landings grew the suite vs
`5103b26b` (no new `tests/test_*.py` modules; existing modules grew:
`test_cf_cost_verify.py` 7→8, `test_cf_propose_thesis.py` 10→11,
`test_immutable_artifact.py` 18→22, `test_occupancy_audit.py` 15→16;
Worker source `it(` / `test(` **145 → 146**, one new
`http.test.ts` `it("denies when token unbound")`). That is **not** a
consolidation win and **not** a GO. YAML under `specs/research_logics` is
**0**; compiled catalog identity is not those files.
...
### 2. Dual-runtime echo husks — **OPEN**
...
- `tests/test_ingestion_secrets_worker_contract.py` (4 collected) still
  ...
  only when that Worker test owns them.
...
- Local `verify_ci` PASS documented at `5103b26b`
  ([`P632_verify_ci_5103b26b.md`](P632_verify_ci_5103b26b.md): **1492
...
- Collect-only **succeeded**. Collected **1503**. Brief `tests_before` ~**1282**.
  Prior inventory **1496** at `5103b26b`. `tests_after` is **not invented**.
  The +7 vs `5103b26b` is not a win.
- Collect-only ≠ green pytest. This snapshot does not re-run host PEM isolation
  or a full `tests/` pass. A later `verify_ci` runtime at `5103b26b` is a
  different SHA and is not this collect and is **not** merge-gate.
- Worker **20** is a file count, not `vitest --collect`. Source `it(` / `test(`
  count **146** is also not vitest collect.
...
- Generated IR **Worker codec body** is not a generated **Python codec body**
  (**OPEN**). Dual-runtime echo husks (**OPEN**). `verify_ci` is local
  authority, not merge-gate (**OPEN**). Catalog compact is optional **HOLD**,
  not missing-CLOSE.
...
This file is a now-inventory receipt at `3b64bdfc`, not a pass.
