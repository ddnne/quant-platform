# P632 independent review C revisit — catalog and pilot gates at `2b82ec7d`

**Lane:** independent C (not the implementer; isolation worktree `grok/p632-ind-C-revisit-2b82ec7d`; docs only)  
**HEAD:** `2b82ec7d` (`2b82ec7d26f26464ac5ce8e4f53d5f6a039117a6`) (`ops: cf_premium_backfill uses shared official-index HTML reader`)  
**Prior freeze:** `242c2484` (`242c2484e9307f9163b13fc603ad90f67c6a0919`) — [`P632_ind_C_revisit_242c2484.md`](P632_ind_C_revisit_242c2484.md)  
**Earlier freezes:** `0a8ced34` — [`P632_ind_C_revisit_0a8ced34.md`](P632_ind_C_revisit_0a8ced34.md); `5103b26b` — [`P632_ind_C_revisit_5103b26b.md`](P632_ind_C_revisit_5103b26b.md); `ed94d504` — [`P632_ind_C_revisit_ed94d504.md`](P632_ind_C_revisit_ed94d504.md); `67fcbd7c` — [`P632_ind_C_revisit_67fcbd7c.md`](P632_ind_C_revisit_67fcbd7c.md); `40d1aa90` — [`P632_ind_C_revisit_40d1aa90.md`](P632_ind_C_revisit_40d1aa90.md); `f224e7e` — [`P632_ind_C_revisit_f224e7e.md`](P632_ind_C_revisit_f224e7e.md); `07b4435` — [`P632_ind_C_revisit.md`](P632_ind_C_revisit.md); `3ab87d0` — [`P632_ind_C_catalog_pilot.md`](P632_ind_C_catalog_pilot.md)  
...
**Catalog/pilot gates at `2b82ec7d` are unchanged vs `242c2484`.** Identity blobs (eval_flags, overlay load, compiler, freeze n, wrangler Mass/Phase 7, reconstitution apply, Phase 7 `start()`, factory, unique22 park, leftover occupancy Worker policy) are the same git object. Research Python that moved in this window is Evaluation IR (emitters / generated types), not yaml n, compiled n, overlay, `+N`, apply, unique22 park, leftover occupancy math, Phase 7 `start()`, or Mass flags. Live counts were re-measured at this HEAD, not copied.
...
HEAD **still matches** the previous PASS/HOLD scoreboard at `242c2484`. Counts, `+N` stops, reconstitution apply, Phase 7, Mass flags, YAML overlay fail-closed, unique22 park occupancy, leftover occupancy HOLD, and compact-catalog-not-done are **unchanged**. P0 unresolved for catalog/pilot is **0**. That is **not** Phase 7 GO.

| Gate | At `242c2484` | At `2b82ec7d` | Live? |
|------|---------------|---------------|-------|
...
| `projection_age_seconds` | 187972 (~52.2 h) |
...
[`original_plan_gap.md`](original_plan_gap.md) register — **unchanged** at this HEAD (banner now names `242c2484`, this window’s pointer only; this lane does not rewrite it; body freeze remains `e927b97`):
...
## What moved `242c2484` → `2b82ec7d` (this lane)

`git rev-list --count 242c2484..2b82ec7d` = **16**. Identity blobs for yaml n, compiled n, overlay, `+N`, reconstitution apply, Phase 7 `start()`, unique22 park, leftover occupancy Worker policy, and Mass flags are **identical** (same git object):
...
Catalog-adjacent research files **did** move. They are not a catalog/pilot gate change:

| File | Commit | Effect on this lane |
|------|--------|---------------------|
| `packages/product/research/evaluation_ir.py` (+ `evaluation_ir_emit.py`, `evaluation_ir_types.generated.py`) | `54c1f472` `research: extract Evaluation IR codec emitters from façade`; `e20be4d9` `research: emit Evaluation IR python types from schema.json` | Evaluation IR Python emitters/types from `schema.json`. Independent B / grading authority. Not yaml n, compiled n, overlay, `+N`, reconstitution apply, unique22 park, leftover occupancy, Phase 7, or Mass. |
...
|--------|---------------------|
| `e9b32084` `docs: independent review C catalog/pilot revisit at 242c2484` | Prior C revisit text. Not a gate change. |
| `31e19ca8` `docs: banner original-plan-gap register still holds at 242c2484` | Pointer only. Body freeze remains `e927b97`. Not a gate change. |
| `54c1f472` / `e20be4d9` Evaluation IR emitters / python types | Independent B / IR. Not catalog/pilot n. |
| `2323f6a5` / `5a8e78c2` / `2b82ec7d` (official-index HTML reader / `--index-text` / shared reader in `cf_premium_backfill`) | Independent A / ingest. Not yaml n, compiled n, overlay, `+N`, apply, Phase 7, or Mass. |
| `908e8ef4` `test: secrets proxy boundary is Worker unit not Python grep` | Independent B / Worker unit. Not catalog/pilot. |
| remaining docs / review-index / Independent A–B revisit at `242c2484` / wave-9 status / test inventory / `verify_ci` result | Not yaml n, compiled n, AND stop, reconstitution apply, Phase 7, Mass, overlay, unique22 park, or leftover occupancy. |
...
- **severity:** was P1 at `3ab87d0`; **status HOLD** at `242c2484` and **HOLD** at `2b82ec7d`
...
- **status:** OPEN (not live; yaml n=0; overlay env unset). Same as `242c2484`.
...
- **status:** OPEN (factorize) / HOLD (growth stopped; `start()` OFF; compact matrix not done). Same as `242c2484`.

---

## HOLD confirmations (re-measured at `2b82ec7d`; same as `242c2484`)

### YAML / AND `+N` growth — stopped

`packages/product/research/eval_flags.py:7-12` (blob unmoved vs `242c2484`):
