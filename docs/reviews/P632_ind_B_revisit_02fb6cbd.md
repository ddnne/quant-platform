...
| `projection_age_seconds` | 188684 (~52.41 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `2b82ec7d`. Empty diff on `platform/workers/ci-aggregate`. |
| Does `verify_ci` include `ci-aggregate` (7 workers)? | **Yes.** `WORKERS` has **7** paths; `tests/test_verify_ci_script.py` pins `len(WORKERS) == 7` and `for name in WORKERS: assert name in src`. Empty diff vs `2b82ec7d` on `scripts/verify_ci.sh` and the pin file. |
| Are GitHub check-runs still 0 live at `02fb6cbd`? | **Yes.** HEAD `02fb6cbd`, prior `2b82ec7d`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = []`. Actions workflows **0**. |
...
| Do premium Worker test files now include catalog / availability / collection_receipts? | **Yes.** At `2b82ec7d` premium tests were `fetch_jq` / `identity` / `index` / `retry_jitter` only. This HEAD adds `catalog.test.ts`, `availability.test.ts`, `collection_receipts.test.ts`. That is a local `npm test` lane, **not** a merge-gate close. |
...
## Scoreboard vs `2b82ec7d` / `242c2484` / `3b64bdfc` / `5103b26b` / `ed94d504` / `67fcbd7c` / `40d1aa90` / `f224e7e` / `07b4435` / `a48c600`

| ID | Topic | Sev | At `a48c600` | At `07b4435` | At `f224e7e` | At `40d1aa90` | At `67fcbd7c` | At `ed94d504` | At `5103b26b` | At `3b64bdfc` | At `242c2484` | At `2b82ec7d` | At `02fb6cbd` |
|----|-------|-----|--------------|--------------|--------------|---------------|---------------|---------------|---------------|---------------|---------------|---------------|---------------|
| P632B-01 | `verify_all` skip vs `verify_ci` authority | P0 | OPEN (scripts split FIXED; `ci-aggregate` absent from `verify_ci`; merge gate not `verify_ci`) | OPEN (`ci-aggregate` in `verify_ci` FIXED; GitHub still does not run `verify_ci`) | OPEN (IR jsonschema + `wrangler types --check` in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`npm run types -- --check` + ALLOWED_FIELDS freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (seven-worker `verify_ci` unchanged vs `40d1aa90`; merge gate still six lane receipts) | OPEN (IR encode-keys freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (IR codec-generated freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`verify_ci.sh` empty vs `5103b26b`; test pins now require freeze **calls**; merge gate still six lane receipts) | OPEN (Python IR codec generated + `assert_evaluation_ir_codec_py_frozen` in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (Python IR **types** generated + `assert_evaluation_ir_types_py_frozen` in `verify_ci` FIXED; merge gate still six lane receipts) | **OPEN** (premium Worker unit tests catalog/availability/collection_receipts **added**; seven-worker `verify_ci` empty vs `2b82ec7d`; merge gate still six lane receipts) |
...
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | same | same (no `catalog_kind` in mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | **same** (empty grep for `catalog_kind` under mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | FIXED | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |

Independent P0 unresolved: **2**. Tree-level holes named at freeze for receipt auth and seventh-worker `verify_ci` stay closed. Overlay CLI put on `default_r2_put` is closed in tree. Python IR codec **and** types remain present and frozen in local `verify_ci`. Premium Worker unit tests now execute catalog / availability / collection_receipts instead of Python grep. The live merge gate still has **no honest producer**. Do not invent Coverage COMPLETE, Projection FRESH, B0 PASS, READY, or CI green.

Window vs `2b82ec7d`: `git rev-list --count 2b82ec7d..02fb6cbd` = **15**. CI-surface deltas that are **not** a P0 close: `23a5cbb9` (`catalog.test.ts` — Worker unit, not Python grep); `8fc13e24` (`availability.test.ts`); `5ac9cce1` (identity JST clocks in Worker unit); `0383311f` (raw-page retain in `index.test.ts`); `0fee1b1e` (`collection_receipts.test.ts` + coverage-segment plan in `index.test.ts`). Remaining 10 commits are docs, including `5e4abd42` (Independent B revisit at `2b82ec7d`). `platform/workers/ci-aggregate`, `scripts/ci_aggregate_first_deploy.sh`, `platform/workers/research-ai-gateway`, `platform/workers/research-mass-eval/src/ai_gateway_client.ts`, `platform/workers/research-mass-eval/src/http.ts`, `packages/product/research/r2_io.py`, `scripts/verify_ci.sh`, and `tests/test_verify_ci_script.py` are **empty diffs** against `2b82ec7d`. Print-only `scripts/ci_aggregate_first_deploy.sh` is **not** a producer.
...
**severity:** P0  
**affected:** `scripts/verify_ci.sh`; `scripts/verify_all.sh`; `tests/test_verify_ci_script.py`; `docs/ci/workers_builds.md`; `scripts/README.md`; `tests/README.md`; GitHub required context `ci-aggregate`; `platform/workers/ingestion-premium/src/{catalog,availability,collection_receipts,identity,index}.test.ts`  
**status:** **OPEN** (seven workers + IR schema + types `--check` honoring `scripts.types` + ALLOWED_FIELDS freeze + encode-keys freeze + TS codec-generated freeze + Python codec-generated freeze + Python types-generated freeze in `verify_ci` **FIXED**; premium catalog/availability/collection_receipts Worker unit tests **added**; merge-gate producer still six `npm test` receipts)

### FIXED (must stay)

`scripts/verify_ci.sh` still lists seven Workers (`WORKERS` array empty-diff vs `2b82ec7d`):
...
Python Evaluation IR **types** file remains **present** and frozen (empty diff vs `2b82ec7d` on `verify_ci.sh`):
...
This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_2b82ec7d.md`](P632_verify_ci_2b82ec7d.md) is a later local PASS at `2b82ec7d`, not a GitHub status and not this SHA.

### Delta vs `2b82ec7d` (not a P0 close)

Premium Worker `npm test` now includes catalog / availability / collection_receipts as **Worker unit tests**, not Python source-grep:

| File at `02fb6cbd` | At `2b82ec7d` | Role |
|--------------------|---------------|------|
| `platform/workers/ingestion-premium/src/catalog.test.ts` | **absent** | JSON contract dataset_id set, n=23, Worker ids == contract ids (`23a5cbb9`) |
| `platform/workers/ingestion-premium/src/availability.test.ts` | **absent** | `policyForDataset` / `pickAvailableAt` wrapper vs contract (`8fc13e24`) |
| `platform/workers/ingestion-premium/src/collection_receipts.test.ts` | **absent** | `writeRequiredCoverageSegment` UNKNOWN insert; event_driven null query units (`0fee1b1e`) |
| `identity.test.ts` | present | JST clocks (`15:00` / `15:30` / `11:30`) now Worker unit (`5ac9cce1`) |
| `index.test.ts` | present | raw-page retain (`0383311f`) + coverage-segment plan (`0fee1b1e`) added |

Python grep pins removed: `tests/test_phase35_availability.py` (catalog/wrapper source grep), `tests/test_phase6_data_access.py` (`test_worker_retains_every_raw_page_and_scopes_tokens`), `tests/test_phase61_coverage_v2.py` (`test_worker_plans_non_event_query_units_before_collection`).
...
PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `02fb6cbd`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “premium Worker now unit-tests catalog/availability/collection_receipts” or “`ci-aggregate` is in `verify_ci.sh`” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `2b82ec7d` on `platform/workers/ci-aggregate`:
...
`scripts/ci_aggregate_first_deploy.sh` is an **empty diff** vs `2b82ec7d`. Default is dry-run / print-only. `--apply` without `CONFIRM_CI_AGGREGATE_CREATE=1` fails closed. With the confirm env, the script **still prints** `npx wrangler deploy` as a comment and does **not** exec it. Tests pin no live secret values. That helper is operator text, not a producer. This isolation turn did **not** run `--apply` and did **not** deploy.

### Live GitHub / Cloudflare (this isolation turn)

Fetched 2026-08-23T16:56Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
GET /repos/ddnne/quant-platform/commits/2b82ec7d…/status    → total_count: 0, state: pending
GET /repos/ddnne/quant-platform/commits/2b82ec7d…/check-runs → total_count: 0
GET /repos/ddnne/quant-platform/commits/02fb6cbd…/status    → total_count: 0, state: pending
GET /repos/ddnne/quant-platform/commits/02fb6cbd…/check-runs → total_count: 0
GET /repos/ddnne/quant-platform/actions/workflows           → total_count: 0
PR #1  head=02fb6cbd  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=[]
```
...
1. has never been posted on `origin/main`, this reviewed SHA `02fb6cbd`, prior revisit `2b82ec7d`, or earlier Independent B SHAs,
...
**status:** **OPEN / HOLD** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `2b82ec7d` on `ai_gateway_client.ts`. [`P632B_03_gateway_token_service_binding_hold.md`](P632B_03_gateway_token_service_binding_hold.md) is **HOLD, not closed**.
...
**status:** mixed. Production budget path empty vs `2b82ec7d`.
...
`89415105` still pins: created ledger has **zero** occupancy; `budget_id` presence is not a reserve. Comment: “Live Cloudflare Durable Object occupancy is unproven.” That is honesty, not a live occupancy proof and not a P1 close. Empty production-path diff vs `2b82ec7d`.
...
**severity:** P0 Worker overwrite **FIXED**; overlay CLI TOCTOU **FIXED in tree**; P1 leftover callers **OPEN** (unchanged vs `2b82ec7d` on production writers)  
...
**status:** mixed. Worker `onlyIf` empty vs `2b82ec7d`. `r2_io.py` empty vs `2b82ec7d`. Overlay CLI put remains closed. Live R2 POST unproven.

### FIXED (Worker 409)

`putJsonCreateOnly` (`http.ts:113-138`): `onlyIf: { etagDoesNotMatch: "*" }`; existing key with a different digest → 409. Child conflict still does not mint a manifest. Empty diff vs `2b82ec7d` on `http.ts`.
...
`default_r2_put` remote path **always** raises `WORKER_CHILDREN_THEN_MANIFEST_ERROR` (`r2_io.py:361`). Overlay `QP_ALLOW_PYTHON_R2_PUT=1` does **not** resurrect `wrangler r2 object put`. Empty diff vs `2b82ec7d` on `r2_io.py`.
...
Job-artifact callers this HEAD that use `put_research_artifact`: `cf_mass_eval_job`, `cf_mass_eval_stage`, `cf_mass_eval_run`, `cf_daily_path_job`, `cf_cost_verify`, `cf_propose_thesis`, `occupancy_audit`. Unchanged vs `2b82ec7d`.
...
Product/research remaining callers are unchanged vs `2b82ec7d` (definition + dry_run). Mentions of the name in `r2_io.py` comments / `__all__` remain.
...
- Premium Worker catalog/availability/collection_receipts tests close P632B-01. They are local `npm test` coverage, not the merge gate.
