# Phase 6.3.2 P — test inventory at `b1605c36`

**Kind:** read-only inventory. Does **not** delete tests. Does **not** invent
`tests_after`. Does **not** flip GO.  
**SHA counted:** `b1605c36e4a5f2c5048264d71baadea8589c4ed4` (`b1605c36`)  
`docs: P632 verify_ci code-lane result at b5f6f2de`  
**Isolation branch:** `docs/p632-test-inventory-b1605c36` off
`origin/grok/phase63-ci-source-closure` (did **not** checkout
`grok/phase63-ci-source-closure` by name). Origin still this SHA.  
...
(`cf7da56c`, collected **1504**); [`P632_test_inventory_b5f6f2de.md`](P632_test_inventory_b5f6f2de.md)
(`b5f6f2de`, collected **1504**).  
...
Brief §P asked `tests_before` ~ **1282** collected. This file re-counted
`pytest --collect-only` against the `b5f6f2de` freeze dump plus the one
source-confirmed new row at this SHA. It does **not** invent `tests_after`.

---

## Method

| Surface | How counted (this SHA `b1605c36`) |
|---------|-----------------------------------|
| Python collected | `/Users/taku/GitHub/quant-platform/.venv/bin/python` is Python 3.11.15, pytest 9.1.1 (`pyproject.toml` `addopts = "-q"`). Prior freeze dump at `b5f6f2de`: quiet `-q` 154 per-file counts summing to **1504**; addopts-cleared `1504 tests collected in 4.78s` (`/Users/taku/tmp/qp-p632-collect-b5f6f2de.txt` / `-nquiet.txt`). This SHA adds **one** unparametrized collected row in existing `tests/test_phase35_coverage_daily.py` (`test_C12_guarded_ids_match_catalog_addon_group`, `de8f87bf`; that file **30 → 31**). No `tests/test_*.py` module add or delete. Collected **1505**. `PYTHONPATH` unset. Later live checkout `grok/p632-wave14-status` is **not** this count. |
| `tests/test_*.py` files | `git ls-files 'tests/test_*.py'` equivalent at this SHA (no `__pycache__`) |
...
| Metric | Brief §P (`tests_before`) | `b5f6f2de` inventory | This SHA (`b1605c36`) |
|--------|--------------------------:|---------------------:|----------------------:|
| **tests_before (collected)** | ~**1282** | — | — |
| **tests_after (collected)** | *not invented* | *not invented* | *not invented* |
| **collected now (actual)** | — | **1504** | **1505** |
| `tests/test_*.py` files | — | **154** | **154** |
| Worker `*.test.ts` | — | **25** | **30** |
| Worker `*.test.mjs` | — | **4** | **4** |
| Worker first-party test files | — | **29** | **34** |
| Worker source `it(` / `test(` | — | **195** | **213** (not vitest collect) |
| `specs/research_logics` YAML | — | **0** | **0** (dir exists; only `README.md`) |

`1505 > 1504 = 1504 < 1505 < 1509 > 1506 > 1503 > 1496 > 1470 > 1448 > 1426 > 1379 > 1282`.
Python collected is **+1** vs `b5f6f2de` with `tests/test_*.py` **154** unchanged
(no module add). The extra collected row is
`tests/test_phase35_coverage_daily.py::test_C12_guarded_ids_match_catalog_addon_group`
(`de8f87bf`: coverage addon guard ids from catalog `list_datasets("addon")`,
not a second hardcoded list). That +1 is **not** a win. Later landings vs
`b5f6f2de` added five Worker first-party files (**29 → 34**; `it(` / `test(`
**195 → 213**):

- `ingestion-premium/src/ops_cold_archive.test.ts` (4 `it(`; `9956ab51`)
- `ingestion-premium/src/ops_prune_changelog.test.ts` (3 `it(`; `9b0582d4`)
- `ingestion-premium/src/ops_parquet_manifest.test.ts` (3 `it(`; `359b2566`)
- `ingestion-premium/src/ops_artifacts_plan.test.ts` (5 `it(`; `329f3959`)
- `ingestion-premium/src/master_scd2/write.test.ts` (3 `it(`; `ee167188`)

Sibling landing that did **not** add a Python module: `de8f87bf` (C12 catalog
addon identity freeze in the existing daily-coverage file). Loose `\bit(` /
`\btest(` is **214** because `index.test.ts` also contains `.test(put.key)`;
**213** is the line-start `it(` / `test(` count, comparable to `b5f6f2de`
**195**. Worker file-count growth is **not** a win. YAML under
`specs/research_logics` is **0**; compiled catalog identity is not those files.

`persist_records.ts` still has **no** `*.test.ts` on this SHA. Do not invent
`persist_records.test.ts` (or later mass-eval `ai_gateway_client` / `panels` /
`eval_orchestrate` / `metrics` units) as present here.
...
### 5. `persist_records` — **OPEN**; `master_scd2` / `ops_*` — **CLOSED**

**Replacement asked:** Worker-unit persist / SCD2 / ops surfaces instead
of leaving production modules without first-party tests.

At this SHA (`origin/grok/phase63-ci-source-closure` `b1605c36`):

| Module | Exports | First-party `*.test.ts` |
|--------|---------|-------------------------|
...
| `ingestion-premium/src/master_scd2/write.ts` | `payloadToMasterRecord`, `writeMasterScd2` | **3 `it(`** (`write.test.ts`; `ee167188`) |
| `ingestion-premium/src/ops_artifacts_plan.ts` | `handleArtifactsJoinPlan` | **5 `it(`** (`ops_artifacts_plan.test.ts`; `329f3959`) |
| `ingestion-premium/src/ops_cold_archive.ts` | `handleArchiveCold` | **4 `it(`** (`ops_cold_archive.test.ts`; `9956ab51`) |
| `ingestion-premium/src/ops_parquet_manifest.ts` | `handleParquetManifest` | **3 `it(`** (`ops_parquet_manifest.test.ts`; `359b2566`) |
| `ingestion-premium/src/ops_prune_changelog.ts` | `handlePruneChangelog` | **3 `it(`** (`ops_prune_changelog.test.ts`; `9b0582d4`) |
...
`master_scd2` write and `ops_*` token fail-closed **did** land on this
SHA. `persist_records.ts` still has no `*.test.ts` here. Do not invent
that test as present. Count growth of the five Worker units that landed
is **not** a win and does not close persist.
...
- Local `verify_ci` PASS documented at `b5f6f2de`
  ([`P632_verify_ci_b5f6f2de.md`](P632_verify_ci_b5f6f2de.md): **1500
...
- Collected **1505**. Brief `tests_before` ~**1282**. Prior inventory
  **1504** at `b5f6f2de`. `tests_after` is **not invented**. Python
  `tests/test_*.py` files **154** unchanged. The +1 collected row is a
  catalog-addon identity freeze in an existing file (`de8f87bf`). That is
  **not** a win: `daily_path` liquidity comment grep remains **HOLD**;
  `persist_records` remains untested on this SHA; `verify_ci` is not
  merge-gate. Worker first-party **29 → 34** is count growth, also not a
  win. Returning to collected **1505** (same integer as `02fb6cbd`) is
  **not** a restoration of that earlier suite and **not** GO.
- Collect-only ≠ green pytest. This snapshot does not re-run host PEM isolation
  or a full `tests/` pass. A later `verify_ci` runtime at `b5f6f2de` is a
  different SHA and is not this collect and is **not** merge-gate.
- Worker **34** is a file count, not `vitest --collect`. Source `it(` / `test(`
  count **213** is also not vitest collect.
...
- Leftover `catalog.ts` grep (**CLOSED** `44642dfc`). NK rebuild Python grep
  ...
  `master_scd2` / `ops_*` Worker units **CLOSED** on this SHA. `persist_records`
  still untested (**OPEN**). `verify_ci` is local authority, not merge-gate
...
This file is a now-inventory receipt at `b1605c36`, not a pass.
