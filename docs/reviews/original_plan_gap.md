...
> **Pointer (not a rewrite).** Register still holds at `1b20fe1f`; AND product still stopped (`CATALOG_AND_PLUS_N_STOPPED=True`); remaining original-plan work is still human reconstitution (`RECONSTITUTION_APPLY=False`); Phase 7 still **NO-GO**. The body below is a freeze at `e927b97`.
