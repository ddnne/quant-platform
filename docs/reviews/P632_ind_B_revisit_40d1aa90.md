# Independent review B revisit — at `40d1aa90`

**Reviewer:** independent Grok (isolation worktree; not the implementer)  
**Reviewed HEAD:** `40d1aa90` (`40d1aa9009ca1e7a6bd9fdc2df4d4da4cf92eab4`)  
**Branch at audit:** `grok/p632-ind-B-revisit-40d1aa90` (from `grok/phase63-ci-source-closure`)  
**Prior revisits:** `f224e7e` ([`P632_ind_B_revisit_f224e7e.md`](P632_ind_B_revisit_f224e7e.md)); `07b4435` ([`P632_ind_B_revisit.md`](P632_ind_B_revisit.md)); freeze `a48c600` ([`P632_ind_B_ci_authority.md`](P632_ind_B_ci_authority.md))  
**origin/main:** `b5c326a7f612563f2da4a84f08063a307ec38e0a` (feature HEAD is **not** an ancestor of `main`; not merged)  
**Scope:** re-measure Independent B — CI skip, `ci-aggregate` live producer, tokens, R2, budget — vs HEAD `40d1aa90`. Tree-level receipt auth and seven-worker `verify_ci` already landed before `07b4435`. This turn re-diffs those plus P632B-03..07 against `f224e7e`.
...
| `projection_age_seconds` | 182445 (~50.68 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `f224e7e` (`ff053a3`). |
...
| Are GitHub check-runs still 0 live at `40d1aa90`? | **Yes.** HEAD `40d1aa90`, prior `f224e7e`, `07b4435`, freeze `a48c600`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = null`. Actions workflows **0**. |
...
## Scoreboard vs `f224e7e` / `07b4435` / `a48c600`

| ID | Topic | Sev | At `a48c600` | At `07b4435` | At `f224e7e` | At `40d1aa90` |
|----|-------|-----|--------------|--------------|--------------|---------------|
| P632B-01 | `verify_all` skip vs `verify_ci` authority | P0 | OPEN (scripts split FIXED; `ci-aggregate` absent from `verify_ci`; merge gate not `verify_ci`) | OPEN (`ci-aggregate` in `verify_ci` FIXED; GitHub still does not run `verify_ci`) | OPEN (IR jsonschema + `wrangler types --check` in `verify_ci` FIXED; merge gate still six lane receipts) | **OPEN** (`npm run types -- --check` + ALLOWED_FIELDS freeze in `verify_ci` **FIXED**; merge gate still six lane receipts) |
...
| P632B-03 | Shared `GATEWAY_TOKEN` / `MASS_EVAL_TOKEN` | P1 | OPEN (header mix-up FIXED) | not re-diffed | OPEN (mass-eval still sends `GATEWAY_TOKEN`) | **OPEN** (empty diff vs `f224e7e`: mass-eval still sends `GATEWAY_TOKEN`) |
| P632B-04 | Edge budget double-spend | P0/P1 | FIXED same-DO occupancy; OPEN P1 residuals | not re-diffed | same | **same** (P0 occupancy still FIXED; P1 residuals still OPEN; empty budget-path diff vs `f224e7e`) |
| P632B-05 | R2 child 409 / Python TOCTOU writer | P0/P1 | FIXED Worker 409; OPEN Python writer | not re-diffed | same | **same severity** (Worker `onlyIf` still FIXED; remote Python put now fail-closed without `QP_ALLOW_PYTHON_R2_PUT=1`; callers still write; flag is not `onlyIf`) |
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | **same** (no `catalog_kind` in mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |
...
Window vs `f224e7e`: `git rev-list --count f224e7e..40d1aa90` = **20**. CI-surface deltas that are **not** a P0 close: `d8821197` (`verify_ci` freezes generated Evaluation IR `ALLOWED_FIELDS`); `cb14d67b` (`verify_ci` types `--check` honors `scripts.types` via `npm run types -- --check`); `b65fa1d6` (remote Python R2 put fail-closed without `QP_ALLOW_PYTHON_R2_PUT=1`). `platform/workers/ci-aggregate` is an **empty diff** against `f224e7e`. Tokens / budget / catalog / GHA surfaces other than the R2 env fence are **empty diffs** against `f224e7e`.
...
Since `f224e7e`, authoritative CI got **stricter** (still not the merge gate):

- `d8821197`: Evaluation IR generated `ALLOWED_FIELDS` TS is presence-checked and `assert_evaluation_ir_allowed_fields_ts_frozen`.
- `cb14d67b`: if `package.json` has `scripts.types`, `verify_ci` runs `npm run types -- --check` (not bare `npx wrangler types --check`). Tests pin `--include-runtime` on all seven workers.
...
`tests/README.md` names `scripts/verify_ci.sh` as mandatory local CI; `verify_all.sh` is the helper. `scripts/README.md` now leads with `verify_ci.sh` (the `f224e7e` honesty residual that still called Evaluation IR a **presence** check in the first sentence is rewritten). `docs/ci/workers_builds.md:84-90` still says six-lane `npm test` receipts “skip Python/catalog and are **not** `verify_ci`”. Merge “requires GitHub context `ci-aggregate` after authenticated receipts **and** `verify_ci`”.

This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_f224e7e.md`](P632_verify_ci_f224e7e.md) is a later local PASS at `f113cc05`, not a GitHub status and not this SHA.
...
PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `40d1aa90`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “`ci-aggregate` is in `verify_ci.sh`” or “types `--check` honors `scripts.types` locally” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `f224e7e` on `platform/workers/ci-aggregate`:
...
Fetched 2026-08-23T15:11Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
PR #1  head=40d1aa90  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=null
```
...
1. has never been posted on `origin/main`, this reviewed SHA `40d1aa90`, prior revisit `f224e7e`, prior revisit `07b4435`, or freeze `a48c600`,
...
**status:** **OPEN** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `f224e7e`.
...
**status:** mixed. Empty diff vs `f224e7e` on these files.
...
**status:** mixed. Worker HTTP empty diff vs `f224e7e`. Python putter **stricter** (`b65fa1d6`) but not a fence.
...
`r2_io.py` still documents head-then-put **TOCTOU** and refuses `authoritative=True` (`python_cli_put_is_not_immutable_authority = True`). Callers still put Worker job keys: `cf_mass_eval_run.put_local_fallback_artifacts`, `cf_mass_eval_stage`, `cf_mass_eval_job`. `wrangler r2 object put` has no `onlyIf`.

Delta vs `f224e7e`: remote (non-`dry_run`) Python put now raises unless `QP_ALLOW_PYTHON_R2_PUT=1`. That is fail-closed by default. It is **not** Worker `onlyIf`. An env flag is not a create-if-absent fence. Do not treat this as P632B-05 FIXED.
...
- `scripts/README.md` no longer calls Evaluation IR a presence check; `verify_ci.sh` jsonschema-validates and freezes generated ALLOWED_FIELDS. The `f224e7e` honesty residual on that sentence is closed in prose. Merge gate is still not `verify_ci`.
