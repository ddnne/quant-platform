# Independent review B revisit — at `2b82ec7d`

**Reviewer:** independent Grok (isolation worktree; not the implementer)  
**Reviewed HEAD:** `2b82ec7d` (`2b82ec7d26f26464ac5ce8e4f53d5f6a039117a6`)  
**Branch at audit:** `grok/p632-ind-B-revisit-2b82ec7d` (from `grok/phase63-ci-source-closure`)  
**Prior revisits:** `242c2484` ([`P632_ind_B_revisit_242c2484.md`](P632_ind_B_revisit_242c2484.md)); `3b64bdfc` ([`P632_ind_B_revisit_3b64bdfc.md`](P632_ind_B_revisit_3b64bdfc.md)); `5103b26b` ([`P632_ind_B_revisit_5103b26b.md`](P632_ind_B_revisit_5103b26b.md)); `ed94d504` ([`P632_ind_B_revisit_ed94d504.md`](P632_ind_B_revisit_ed94d504.md)); `67fcbd7c` ([`P632_ind_B_revisit_67fcbd7c.md`](P632_ind_B_revisit_67fcbd7c.md)); `40d1aa90` ([`P632_ind_B_revisit_40d1aa90.md`](P632_ind_B_revisit_40d1aa90.md)); `f224e7e` ([`P632_ind_B_revisit_f224e7e.md`](P632_ind_B_revisit_f224e7e.md)); `07b4435` ([`P632_ind_B_revisit.md`](P632_ind_B_revisit.md)); freeze `a48c600` ([`P632_ind_B_ci_authority.md`](P632_ind_B_ci_authority.md))  
**origin/main:** `b5c326a7f612563f2da4a84f08063a307ec38e0a` (feature HEAD is **not** an ancestor of `main`; not merged)  
**Scope:** re-measure Independent B — CI skip, `ci-aggregate` live producer, tokens, R2, budget — vs HEAD `2b82ec7d`. Tree-level receipt auth and seven-worker `verify_ci` already landed before `07b4435`. This turn re-diffs those plus P632B-03..07 against `242c2484`. Live GitHub check-runs, `main` protection, and `wrangler deployments list --name quant-platform-ci-aggregate` are this-turn measurements. P632B-01 code-lane delta: Python Evaluation IR **types** are generated and frozen in `verify_ci` (`evaluation_ir_types.generated.py` **present**). P632B-03 remains **HOLD** ([`P632B_03_gateway_token_service_binding_hold.md`](P632B_03_gateway_token_service_binding_hold.md) — HOLD, not closed). P632B-05 leftover `default_r2_put(` is still definition + dry_run (plus test pins).
...
| `projection_age_seconds` | 187994 (~52.22 h) |
...
| Question | Answer |
|----------|--------|
| Is `POST /v1/receipts` still unauthenticated **in tree**? | **No.** Unbound `CI_LANE_TOKEN` → HTTP **503**; missing/wrong `X-CI-Lane-Token` → HTTP **401**. Unchanged vs `242c2484`. Empty diff on `platform/workers/ci-aggregate`. |
| Does `verify_ci` include `ci-aggregate` (7 workers)? | **Yes.** `WORKERS` has **7** paths; `tests/test_verify_ci_script.py` pins `len(WORKERS) == 7` and `for name in WORKERS: assert name in src`. Python IR **types** freeze is **added**; worker count is unchanged. |
| Are GitHub check-runs still 0 live at `2b82ec7d`? | **Yes.** HEAD `2b82ec7d`, prior `242c2484`, `origin/main` `b5c326a`: `check-runs.total_count = 0`. Commit statuses `total_count = 0` / `state = pending`. PR #1 `statusCheckRollup = []`. Actions workflows **0**. |
...
## Scoreboard vs `242c2484` / `3b64bdfc` / `5103b26b` / `ed94d504` / `67fcbd7c` / `40d1aa90` / `f224e7e` / `07b4435` / `a48c600`

| ID | Topic | Sev | At `a48c600` | At `07b4435` | At `f224e7e` | At `40d1aa90` | At `67fcbd7c` | At `ed94d504` | At `5103b26b` | At `3b64bdfc` | At `242c2484` | At `2b82ec7d` |
|----|-------|-----|--------------|--------------|--------------|---------------|---------------|---------------|---------------|---------------|---------------|---------------|
| P632B-01 | `verify_all` skip vs `verify_ci` authority | P0 | OPEN (scripts split FIXED; `ci-aggregate` absent from `verify_ci`; merge gate not `verify_ci`) | OPEN (`ci-aggregate` in `verify_ci` FIXED; GitHub still does not run `verify_ci`) | OPEN (IR jsonschema + `wrangler types --check` in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`npm run types -- --check` + ALLOWED_FIELDS freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (seven-worker `verify_ci` unchanged vs `40d1aa90`; merge gate still six lane receipts) | OPEN (IR encode-keys freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (IR codec-generated freeze in `verify_ci` FIXED; merge gate still six lane receipts) | OPEN (`verify_ci.sh` empty vs `5103b26b`; test pins now require freeze **calls**; merge gate still six lane receipts) | OPEN (Python IR codec generated + `assert_evaluation_ir_codec_py_frozen` in `verify_ci` FIXED; merge gate still six lane receipts) | **OPEN** (Python IR **types** generated + `assert_evaluation_ir_types_py_frozen` in `verify_ci` FIXED; merge gate still six lane receipts) |
...
| P632B-06 | Active / Legacy catalog mix | P1 | FIXED Python partition; OPEN Worker/eval mix | not re-diffed | same | same | same (no `catalog_kind` in mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | same (empty grep for `catalog_kind` under mass-eval Worker) | **same** (empty grep for `catalog_kind` under mass-eval Worker) |
| P632B-07 | GitHub Actions presence (must be absent) | P0 if present | FIXED (absent) | FIXED | FIXED | FIXED | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | FIXED (`git ls-files` has no `.github/`; live Actions workflows **0**) | **FIXED** (`git ls-files` has no `.github/`; live Actions workflows **0**) |

Independent P0 unresolved: **2**. Tree-level holes named at freeze for receipt auth and seventh-worker `verify_ci` stay closed. Overlay CLI put on `default_r2_put` is closed in tree. Python IR codec **and** types are present and frozen in local `verify_ci`. The live merge gate still has **no honest producer**. Do not invent Coverage COMPLETE, Projection FRESH, B0 PASS, READY, or CI green.

Window vs `242c2484`: `git rev-list --count 242c2484..2b82ec7d` = **16**. CI-surface deltas that are **not** a P0 close: `e20be4d9` (Python Evaluation IR types generated from `schema.json` into `evaluation_ir_types.generated.py`; `verify_ci` presence-checks that file and calls `assert_evaluation_ir_types_py_frozen()`); `54c1f472` (codec emitters extracted from the Python façade — local freeze plumbing, not GitHub). `platform/workers/ci-aggregate`, `scripts/ci_aggregate_first_deploy.sh`, `platform/workers/research-ai-gateway`, `platform/workers/research-mass-eval/src/ai_gateway_client.ts`, `platform/workers/research-mass-eval/src/http.ts`, and `packages/product/research/r2_io.py` are **empty diffs** against `242c2484`. `scripts/verify_ci.sh` is an **8-line** types-freeze add, not a merge-gate close. Print-only `scripts/ci_aggregate_first_deploy.sh` is **not** a producer.
...
**status:** **OPEN** (seven workers + IR schema + types `--check` honoring `scripts.types` + ALLOWED_FIELDS freeze + encode-keys freeze + TS codec-generated freeze + Python codec-generated freeze + **Python types-generated freeze** in `verify_ci` **FIXED**; merge-gate producer still six `npm test` receipts)

### FIXED (must stay)

`scripts/verify_ci.sh` still lists seven Workers (`WORKERS` array empty-diff vs `242c2484`):
...
Delta vs `242c2484` (`e20be4d9`): Python Evaluation IR **types** file is **present** and frozen.
...
- Freeze call: `assert_evaluation_ir_types_py_frozen()` (`evaluation_ir_emit.py:790-803`; re-exported from `evaluation_ir.py:47`).
- Tests pin the generated path and the freeze **call** (`tests/test_verify_ci_script.py:63-67,154-158`).

```69:87:scripts/verify_ci.sh
...
This isolation turn did **not** execute `scripts/verify_ci.sh`. Script/static pins only. [`P632_verify_ci_242c2484.md`](P632_verify_ci_242c2484.md) is a later local PASS at `242c2484`, not a GitHub status and not this SHA.
...
PR #1 is `MERGEABLE` / `BLOCKED` because `ci-aggregate` has never posted — not because `verify_ci` ran. Head SHA this turn is `2b82ec7d`. A green `verify_all` or a future PAT `ci-aggregate` success is still **not** a `verify_ci` run. **Do not claim CI green.**

### Why this is still P0

The freeze P0 was merge-gate skip of the Python plane and of the gate Worker. The gate Worker is in `verify_ci`. The merge gate still skips Python / catalog / IR. Do not treat “`ci-aggregate` is in `verify_ci.sh`” or “Python types freeze calls are pinned” as FIXED for merge authority.
...
`POST /v1/receipts` still authenticates. Empty diff vs `242c2484` on `platform/workers/ci-aggregate`:
...
`scripts/ci_aggregate_first_deploy.sh` is an **empty diff** vs `242c2484`. Default is dry-run / print-only. `--apply` without `CONFIRM_CI_AGGREGATE_CREATE=1` fails closed. With the confirm env, the script **still prints** `npx wrangler deploy` as a comment and does **not** exec it. Tests pin no live secret values. That helper is operator text, not a producer. This isolation turn did **not** run `--apply` and did **not** deploy.

### Live GitHub / Cloudflare (this isolation turn)

Fetched 2026-08-23T16:44Z via `gh api` and Wrangler 4.125.0 (account `11233bca08d134a9b738eaa46b9751d9`, logged in as `taku_haga@icloud.com`, subdomain `taku-haga`):
...
PR #1  head=2b82ec7d  mergeable=MERGEABLE  mergeStateStatus=BLOCKED  statusCheckRollup=[]
```
...
1. has never been posted on `origin/main`, this reviewed SHA `2b82ec7d`, prior revisit `242c2484`, or earlier Independent B SHAs,
...
**status:** **OPEN / HOLD** (cross-header substitute **FIXED**; shared secret on the caller **OPEN**). Empty diff vs `242c2484` on `ai_gateway_client.ts`. [`P632B_03_gateway_token_service_binding_hold.md`](P632B_03_gateway_token_service_binding_hold.md) is **HOLD, not closed**.
...
**status:** mixed. Production budget path empty vs `242c2484`.
...
`89415105` still pins: created ledger has **zero** occupancy; `budget_id` presence is not a reserve. Comment: “Live Cloudflare Durable Object occupancy is unproven.” That is honesty, not a live occupancy proof and not a P1 close. Empty production-path diff vs `242c2484`.
...
**severity:** P0 Worker overwrite **FIXED**; overlay CLI TOCTOU **FIXED in tree**; P1 leftover callers **OPEN** (unchanged vs `242c2484` on production writers)  
...
**status:** mixed. Worker `onlyIf` empty vs `242c2484`. `r2_io.py` empty vs `242c2484`. Overlay CLI put remains closed. Live R2 POST unproven.

### FIXED (Worker 409)

`putJsonCreateOnly` (`http.ts:113-138`): `onlyIf: { etagDoesNotMatch: "*" }`; existing key with a different digest → 409. Child conflict still does not mint a manifest. Empty diff vs `242c2484` on `http.ts`.
...
`default_r2_put` remote path **always** raises `WORKER_CHILDREN_THEN_MANIFEST_ERROR` (`r2_io.py:361`). Overlay `QP_ALLOW_PYTHON_R2_PUT=1` does **not** resurrect `wrangler r2 object put`. Empty diff vs `242c2484` on `r2_io.py`.
...
Job-artifact callers this HEAD that use `put_research_artifact`: `cf_mass_eval_job`, `cf_mass_eval_stage`, `cf_mass_eval_run`, `cf_daily_path_job`, `cf_cost_verify`, `cf_propose_thesis`, `occupancy_audit`. Unchanged vs `242c2484`.
...
Product/research remaining callers are unchanged vs `242c2484` (definition + dry_run). Mentions of the name in `r2_io.py` comments / `__all__` remain.
