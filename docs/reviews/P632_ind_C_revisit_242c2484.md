# P632 independent review C revisit — catalog and pilot gates at `242c2484`

**Lane:** independent C (not the implementer; isolation worktree `grok/p632-ind-C-revisit-242c2484`; docs only)  
**HEAD:** `242c2484` (`242c2484e9307f9163b13fc603ad90f67c6a0919`) (`docs: §10 remaining mixed at c9764ff4 after python IR codec`)  
**Prior freeze:** `0a8ced34` (`0a8ced3487b490972ceda054bbf6f2a305dad147`) — [`P632_ind_C_revisit_0a8ced34.md`](P632_ind_C_revisit_0a8ced34.md)  
**Earlier freezes:** `5103b26b` — [`P632_ind_C_revisit_5103b26b.md`](P632_ind_C_revisit_5103b26b.md); `ed94d504` — [`P632_ind_C_revisit_ed94d504.md`](P632_ind_C_revisit_ed94d504.md); `67fcbd7c` — [`P632_ind_C_revisit_67fcbd7c.md`](P632_ind_C_revisit_67fcbd7c.md); `40d1aa90` — [`P632_ind_C_revisit_40d1aa90.md`](P632_ind_C_revisit_40d1aa90.md); `f224e7e` — [`P632_ind_C_revisit_f224e7e.md`](P632_ind_C_revisit_f224e7e.md); `07b4435` — [`P632_ind_C_revisit.md`](P632_ind_C_revisit.md); `3ab87d0` — [`P632_ind_C_catalog_pilot.md`](P632_ind_C_catalog_pilot.md)  
...
**Catalog/pilot gates at `242c2484` are unchanged vs `0a8ced34`.** Identity blobs (eval_flags, overlay load, compiler, freeze n, wrangler Mass/Phase 7, reconstitution apply, Phase 7 `start()`, factory, unique22 park, leftover occupancy Worker policy) are the same git object. One research Python file moved in this window (`evaluation_ir.py` + generated codec); it is Evaluation IR, not yaml n, compiled n, overlay, `+N`, apply, unique22 park, leftover occupancy math, Phase 7 `start()`, or Mass flags. Live counts were re-measured at this HEAD, not copied.
...
HEAD **still matches** the previous PASS/HOLD scoreboard at `0a8ced34`. Counts, `+N` stops, reconstitution apply, Phase 7, Mass flags, YAML overlay fail-closed, unique22 park occupancy, leftover occupancy HOLD, and compact-catalog-not-done are **unchanged**. P0 unresolved for catalog/pilot is **0**. That is **not** Phase 7 GO.

| Gate | At `0a8ced34` | At `242c2484` | Live? |
|------|---------------|---------------|-------|
...
| `projection_age_seconds` | 187001 (~51.9 h) |
...
[`original_plan_gap.md`](original_plan_gap.md) register — **unchanged** at this HEAD (banner still names `0a8ced34`, prior-window pointer only; this lane does not rewrite it; body freeze remains `e927b97`):
...
## What moved `0a8ced34` → `242c2484` (this lane)

`git rev-list --count 0a8ced34..242c2484` = **16**. Identity blobs for yaml n, compiled n, overlay, `+N`, reconstitution apply, Phase 7 `start()`, unique22 park, leftover occupancy Worker policy, and Mass flags are **identical** (same git object):
...
One catalog-adjacent research file **did** move. It is not a catalog/pilot gate change:

| File | Commit | Effect on this lane |
|------|--------|---------------------|
| `packages/product/research/evaluation_ir.py` (+ `evaluation_ir_codec.generated.py`) | `c9764ff4` `research: emit Evaluation IR python codec from schema.json` | Evaluation IR Python codec from `schema.json`. Independent B / grading authority. Not yaml n, compiled n, overlay, `+N`, reconstitution apply, unique22 park, leftover occupancy, Phase 7, or Mass. |
...
|--------|---------------------|
| `07f98f40` `docs: independent review C catalog/pilot revisit at 0a8ced34` | Prior C revisit text. Not a gate change. |
| `505051a2` `docs: banner original-plan-gap register still holds at 0a8ced34` | Pointer only. Body freeze remains `e927b97`. Not a gate change. |
| `c9764ff4` Evaluation IR python codec | Independent B / IR. Not catalog/pilot n. |
| `2cbd894d` `ops: BackfillPlanner passes index_text into required segments` | Independent A / ingest. Not yaml n, compiled n, overlay, `+N`, apply, Phase 7, or Mass. |
| `a20d14d4` / `82ef0f7b` (premium JQ fetch extract / retry jitter) | Independent B / Worker extract. Not catalog/pilot. |
| remaining docs / review-index / Independent A–B revisit at `3b64bdfc` / wave-8 status / test inventory / `GATEWAY_TOKEN` HOLD / `verify_ci` inventory | Not yaml n, compiled n, AND stop, reconstitution apply, Phase 7, Mass, overlay, unique22 park, or leftover occupancy. |
...
- **severity:** was P1 at `3ab87d0`; **status HOLD** at `0a8ced34` and **HOLD** at `242c2484`
...
- **status:** OPEN (not live; yaml n=0; overlay env unset). Same as `0a8ced34`.
...
- **status:** OPEN (factorize) / HOLD (growth stopped; `start()` OFF; compact matrix not done). Same as `0a8ced34`.

---

## HOLD confirmations (re-measured at `242c2484`; same as `0a8ced34`)

### YAML / AND `+N` growth — stopped

`packages/product/research/eval_flags.py:7-12` (blob unmoved vs `0a8ced34`):
