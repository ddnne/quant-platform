...
- **冪等性**: 各テーブルの自然キーを `PRIMARY KEY` とし `INSERT OR REPLACE` で upsert。同一日の再実行で重複行はできない。
...
- ページネーション: `pagination_token`（応答キーは `pagination_key` の場合もあるため両方を見る）。
- リトライ: 429/5xx を指数バックオフで再試行（`ingestion/common/retry.py`）。
...
| `jquants_daily_bars` | (source, code, date) | 当日 15:00 JST（引け） | 取得時刻（仮） |
...
終了コード: `0`=取得/登録あり, `1`=予期せぬエラー, `2`=何も実行せず（CF ランタイム または全ソース skip）。
