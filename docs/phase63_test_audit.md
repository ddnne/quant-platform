...
| `test_catalog_yaml_parity.py` | Freeze file-count | Identity set `yaml==py==constants` is Invariant. Two full walks of **2 254** YAML files for `go:` / `family:` / `theme:` are freeze. Secondary: Dual-runtime `cheap_pb` body grep. |
...
| `test_immutable_artifact.py` | Invariant | **Keep.** Content-digest create-if-absent. |
...
| `test_lane_e_ops_auto_projection.py` | Representative | Auto projection after sync. |
...
| `test_unique_logic_event_filters.py` | Dual-runtime policy echo | YAML leftover vs lifted is Invariant. Worker `daily_path.ts` leftover grep **echoes** `combo_gates.test.ts` — keep YAML half; see candidates. |
...
| `test_catalog_yaml_parity.py` raw regex loop ×2 254 YAML, twice | Freeze file-count | **Document only** | Parsed `spec.go is not True` already runs on load. Raw `family:` / `theme:` scans may still be the only copy. Collapse to **one** pass later; do not drop identity set-equality. |
...
| `test_unique_logic_event_filters.py` Worker leftover grep | Dual-runtime policy echo | Document | YAML leftover-vs-lifted is unique. Worker slice duplicates `combo_gates`. Next lane: drop grep, keep YAML. |
| `test_catalog_yaml_parity.py::test_event_cheap_pb_gate_in_combo_and_yaml` Worker body regex | Dual-runtime policy echo | Document | Behavior lives in `combo_gates`. Keep until a Worker test asserts event path does not read `extras.cheapPb`. |
| `test_ops_projection_meta.py` | Representative | Document merge | 2 tests; could live in `test_ops_projection_publish.py`. Not merged (lane already used 2-file budget). |
