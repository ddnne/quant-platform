...
from .jsda.official_index import official_index_days
...
def _plan_required_segments(
    ...
    *,
    ...
    held = _index_text_for_plan(policy, index_text)
    params = inspect.signature(plan_required_segments).parameters
    if "index_text" in params:
        return list(
    ...
    # Ledger on this slice still calendar-walks official_archive_day.
    # Missing index_text is fail-closed empty, never a weekend COMPLETE walk.
    if held is None:
        return []
    ...
    segments: list[Any] = []
    for day_s in official_index_days(policy.dataset_id, held):
        day = date.fromisoformat(day_s)
        if day < start or day > end:
            continue
        expected_items = None
        if expected_items_by_segment is not None:
            expected_items = expected_items_by_segment.get(day_s)
