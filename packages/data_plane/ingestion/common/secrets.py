...
(``POST {proxy}/v1/proxy/jquants``) authenticated with a narrowly scoped
``JQUANTS_PROXY_TOKEN``. The Worker injects the real ``x-api-key``
...
1. environment variables ``JQUANTS_PROXY_URL`` **and**
   ``JQUANTS_PROXY_TOKEN`` (both required);
2. else a single JSON file ``~/.config/quant-platform/jquants_proxy.json``
   with keys ``{"url", "token"}`` (both required);
3. else the split files ``~/.config/quant-platform/jquants_proxy_url`` and
   ``jquants_proxy_token`` (first line, stripped; both required).

A *missing* or *partial* config is not an error — :func:`resolve_proxy_config`
simply returns ``None``. The CLI skips J-Quants unless explicit unsafe local
direct mode was enabled.
...
JQUANTS_URL_FILE = CONFIG_DIR / "jquants_proxy_url"
JQUANTS_TOKEN_FILE = CONFIG_DIR / "jquants_proxy_token"
JQUANTS_JSON_FILE = CONFIG_DIR / "jquants_proxy.json"
JQUANTS_ENV_URL = "JQUANTS_PROXY_URL"
JQUANTS_ENV_TOKEN = "JQUANTS_PROXY_TOKEN"

# Pre-split names remain a read-only compatibility input to this J-Quants
# resolver. They are never accepted for ingestion-run or data-export access.
...
class ProxyConfig:
    ...
    ``JQUANTS_PROXY_TOKEN`` secret; treat it as a credential (do not log).
    """
...
def resolve_proxy_config(
    *,
    ...
    1. environment variables ``JQUANTS_PROXY_URL`` **and**
       ``JQUANTS_PROXY_TOKEN`` (both required);
    2. a single JSON pair file ``jquants_proxy.json`` (both keys required);
    3. the split files ``jquants_proxy_url`` and ``jquants_proxy_token``
       (both required).
    ...
    a clean skip rather than half-working.
    """
    env = os.environ if env is None else env
    base = config_dir or CONFIG_DIR
    canonical_json = base / "jquants_proxy.json" if config_dir else JQUANTS_JSON_FILE
    canonical_url = base / "jquants_proxy_url" if config_dir else JQUANTS_URL_FILE
    canonical_token = base / "jquants_proxy_token" if config_dir else JQUANTS_TOKEN_FILE
    ...
    # 1) canonical J-Quants capability. Partial coordinates fail closed and
    # cannot be mixed with compatibility files.
    jq_env_url = (env.get(JQUANTS_ENV_URL) or "").strip()
    jq_env_token = (env.get(JQUANTS_ENV_TOKEN) or "").strip()
    if jq_env_url or jq_env_token:
        if jq_env_url and jq_env_token:
            return ProxyConfig(url=jq_env_url.rstrip("/"), token=jq_env_token)
        return None
    canonical_cfg = _read_json_pair(canonical_json)
    if canonical_cfg is not None:
        return canonical_cfg
    jq_file_url = _read_first_line(canonical_url)
    jq_file_token = _read_first_line(canonical_token)
    if jq_file_url or jq_file_token:
        if jq_file_url and jq_file_token:
            return ProxyConfig(url=jq_file_url.rstrip("/"), token=jq_file_token)
        return None

    # 2) compatibility env pair — both must be set together.
