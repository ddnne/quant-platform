"""Read-only operational observations over the mutable local control database.

``OpsCurrentReadService`` owns sqlite connections and named operational SQL
reads. Results are never research facts. Missing control tables yield UNKNOWN
domain results rather than evidence.
"""
