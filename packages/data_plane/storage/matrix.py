...
the coverage runner (:mod:`storage.coverage`) and tests.

Pure data — no I/O. The runners that actually execute checks live in
:mod:`storage.coverage`.

Adding a new id? Add it here AND to the doc table. Daily-tier membership is
asserted by ``tests/test_phase35_coverage_matrix.py`` against catalog ids
(not a markdown-count parser).
"""
