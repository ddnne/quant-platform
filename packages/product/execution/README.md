...
expiring one-use SQLite ledger and prints browser/OS creation parameters. Request
and proposal timestamps come only from the module-owned UTC clock; the CLI has
no caller-supplied timestamp option. Its
...
the validated transcript digest into the root activation proposal. Because
`fmt=none` has no trusted attestation root, the proposal records only
`registration_payload_validated=true`, `attestation_state=UNATTESTED`, and a
`PENDING_TRUST_REVIEW` credential with `key_backend=UNATTESTED`. Positive
activation requires a root-reviewed human-witness evidence digest while keeping
that honest backend, or a separate trusted-attestation evidence digest. Neither
is generated by this CLI. Caller-supplied SPKI,
