...
- **Catalog compiler:** `research.catalog_compiler` closed-DSL data artifact + semantic hash. Owns generated `catalog_ids.ts` emit (Python constants remain policy SoT until YAML deletion). YAML still present (not deleted this wave). Persisted `specs/research_catalog/` manifest + migration map; not GO.
