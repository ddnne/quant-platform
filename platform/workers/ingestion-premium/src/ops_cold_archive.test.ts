...
describe("handleArchiveCold fail-closed", () => {
  it("GET with matching token and dataset+before is 405 with no D1 and no R2 put", async () => {
    const { env, sql, r2Puts } = touchingEnv();
    const url = new URL(ARCHIVE_PATH);
    url.searchParams.set("dataset", "markets_calendar");
    url.searchParams.set("before", "2024-01-01");
