...
async function runMassEval(
  ...
  const t0 = Date.now();
  const version = env.MASS_EVAL_VERSION || "research-mass-eval/v96-24bu-p10-sales-invert";
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health" || url.pathname === "/") {
      ...
      return json({
        ...
        version: env.MASS_EVAL_VERSION || "research-mass-eval/v96-24bu-p10-sales-invert",
        wave: env.MASS_EVAL_WAVE || "research-mass-eval",
