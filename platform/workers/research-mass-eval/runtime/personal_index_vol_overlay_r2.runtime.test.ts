...
} from "../src/personal_index_vol_overlay_2023_contract";
import { personalIndexVolOverlayR2Outbound } from "../src/personal_index_vol_overlay_r2";
...
async function storedBytes(key: string) {
  const object = await runtimeEnv.STRUCTURED_BUCKET.get(key);
...
async function familyRequestDigest(
  ...
  return personalIndexVolOverlay2023RequestDigest(
    { job_id: jobId, cohort_id: cohortId, base_job_id: "base-r2", svi_job_id: "svi-r2" },
    inputDigest,
  );
}
...
async function overlayListedInput(
  jobId: string,
  inputKey: string,
  input: PersonalIndexVolOverlay2023InputManifest | PersonalIndexSmileTransport2023InputManifest | PersonalIndexVolOverlay2023AmPmInputManifest | Record<string, unknown>,
) {
  ...
  const rawObj = await seed(rawKey, raw);
  const withEtag = JSON.parse(JSON.stringify(input)) as typeof input;
  const days = (withEtag as { svi: { options: { days: Array<{ objects: Array<{ etag: string; size: number }> }> } } }).svi.options.days;
  days[0]!.objects[0]!.etag = rawObj.etag;
  days[0]!.objects[0]!.size = rawObj.size;
  const inputBytes = new TextEncoder().encode(JSON.stringify(withEtag));
...
function overlaySvi(rawKey: string, rawSize: number) {
  return {
    job_id: "svi-r2",
    ...
    options: { days: [{ date: "2023-01-04", objects: [{ key: rawKey, etag: "raw", size: rawSize, sha256: digest("9") }] }], object_count: 1, total_bytes: rawSize },
  };
}
...
async function fixture() {
  ...
  const input: PersonalIndexVolOverlay2023InputManifest = {
    ...
    base: overlayBase(),
    svi: overlaySvi(rawKey, raw.byteLength),
  ...
  return overlayListedInput(jobId, personalIndexVolOverlay2023InputManifestKey(jobId), input);
}
...
async function put(
  ...
  const key = kind === "manifest"
    ...
    : personalIndexVolOverlay2023ArtifactKey(kind, sha);
  const response = await personalIndexVolOverlayR2Outbound(
    new Request(`http://research.r2/${key}`, {
      method: "PUT",
      headers: { ...fixed.headers, "content-length": String(bytes.byteLength), "x-content-sha256": sha },
      body: bytes,
    }),
    runtimeEnv,
    key,
  );
  return { response, digest: sha, key, bytes };
}
...
async function smileFixture() {
  ...
  return overlayListedInput(jobId, personalIndexSmileTransport2023InputManifestKey(jobId), input);
}
...
async function amPmFixture() {
  ...
  return overlayListedInput(jobId, personalIndexVolOverlay2023AmPmInputManifestKey(jobId), input);
}
...
async function smileAmPmFixture() {
  ...
  return overlayListedInput(jobId, personalIndexSmileTransport2023AmPmInputManifestKey(jobId), input);
}
...
describe("index-vol overlay exact-reference R2 capability", () => {
  ...
  it("requires content-addressed children before the terminal and replays idempotently", async () => {
    ...
    const terminalDoc = {
      ...
      ...authority(fixed),
      runner_version: PERSONAL_INDEX_VOL_OVERLAY_2023_RUNNER_VERSION,
      request_digest: await familyRequestDigest(fixed.jobId, PERSONAL_INDEX_VOL_OVERLAY_2023_COHORT_ID, fixed.inputDigest),
      prepared_panel_key: personalIndexVolOverlay2023ArtifactKey("prepared-panel", panelDigest),
      prepared_panel_sha256: panelDigest,
    ...
    expect(await storedBytes(panel.key)).toEqual(panel.bytes);
    expect(await storedBytes(report.key)).toEqual(report.bytes);
    expect(await storedBytes(terminal.key)).toEqual(terminal.bytes);
  ...
  it("rejects late child artifact creation after an exact FAILED terminal", async () => {
    const fixed = await fixture();
    const terminal = {
      schema_version: personalIndexOverlayFamilyTerminalSchema(PERSONAL_INDEX_VOL_OVERLAY_2023_COHORT_ID),
      status: "FAILED",
      ...authority(fixed),
      runner_version: personalIndexOverlayFamilyRunnerVersion(PERSONAL_INDEX_VOL_OVERLAY_2023_COHORT_ID),
      request_digest: await familyRequestDigest(fixed.jobId, PERSONAL_INDEX_VOL_OVERLAY_2023_COHORT_ID, fixed.inputDigest),
    };
    expect((await put(fixed, "manifest", terminal)).response.status).toBe(201);
    const late = await put(fixed, "prepared-panel", {
      schema_version: "personal-index-vol-overlay-prepared-panel/v1",
      ...authority(fixed),
    });
    expect(late.response.status).toBe(409);
    expect(await late.response.json()).toEqual({ error: "overlay terminal already exists" });
    expect(await runtimeEnv.STRUCTURED_BUCKET.head(late.key)).toBeNull();
  });
});
...
describe("index-smile-transport exact-reference R2 capability", () => {
  ...
  it("rejects v1 overlay schemas on the closed v2 terminal path", async () => {
    ...
    const sha = `sha256:${await sha256Hex(bytes)}`;
    const key = personalIndexSmileTransport2023ArtifactKey("prepared-panel", sha);
    const denied = await personalIndexVolOverlayR2Outbound(
      new Request(`http://research.r2/${key}`, {
        method: "PUT",
        headers: { ...fixed.headers, "content-length": String(bytes.byteLength), "x-content-sha256": sha },
    ...
    expect(await storedBytes(acceptedKey)).toEqual(acceptedBytes);
  });
});
...
describe("index-vol overlay AM/PM exact-reference R2 capability", () => {
  ...
  it("rejects next-close overlay schemas on the AM/PM terminal path", async () => {
    ...
    const bytes = new TextEncoder().encode(JSON.stringify(panelDoc));
    const sha = `sha256:${await sha256Hex(bytes)}`;
    const key = personalIndexVolOverlay2023AmPmArtifactKey("prepared-panel", sha);
    const denied = await personalIndexVolOverlayR2Outbound(
      new Request(`http://research.r2/${key}`, {
        method: "PUT",
        headers: { ...fixed.headers, "content-length": String(bytes.byteLength), "x-content-sha256": sha },
...
describe("AM overlay/smile generic terminal verify-and-shutdown path", () => {
  ...
  it.each([
  ...
  ])("PUT-conflicts then GET-verifies the %s AM family", async (_label, cohortId, makeFixture) => {
    const fixed = await makeFixture();
    ...
    const created = await putFamilyOutbound(key, fixed.headers, terminal);
    expect(created.response.status).toBe(201);
    expect(await storedBytes(key)).toEqual(created.bytes);
    const personal = {
    ...
    const republish = await putFamilyOutbound(key, fixed.headers, terminal);
    expect(republish.response.status).toBe(200);
    expect(await storedBytes(key)).toEqual(created.bytes);
    const personalPut = await putFamilyOutbound(key, personal, terminal);
  ...
  it("rejects malformed AM identity on create-only PUT without writing a terminal", async () => {
    ...
    const familyPut = await putFamilyOutbound(key, fixed.headers, poisoned);
  ...
  it("rejects a request-digest mismatch and COMPLETED terminals without children", async () => {
    ...
    const mismatchPut = await putFamilyOutbound(key, fixed.headers, mismatched);
    expect(mismatchPut.response.status).toBe(400);
    expect(await runtimeEnv.STRUCTURED_BUCKET.head(key)).toBeNull();
    const completed = {
      ...failedTerminal(fixed, cohortId, requestDigest),
      status: "COMPLETED",
      prepared_panel_key: personalIndexVolOverlay2023AmPmArtifactKey("prepared-panel", `sha256:${"a".repeat(64)}`),
    ...
    const missingChildren = await putFamilyOutbound(key, fixed.headers, completed);
